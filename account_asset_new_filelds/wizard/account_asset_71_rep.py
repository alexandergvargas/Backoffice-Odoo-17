...
class account_asset_71_rep(models.TransientModel):
	...
	def _get_sql_71(self,date_fiscal_year_start,date_period_start,date_period_end,company_id,period_code=None):
		sql = """
				...
				case
					when asset.date_at < '%s' then asset.valor_at::numeric
					else 0::numeric
				end
				as campo7,
				case
				...
				as campo8,
				0::numeric as campo9,
				case when asset.f_baja <= '%s' then ((asset.valor_at)*-1)::numeric else 0::numeric end as campo10,
				0::numeric as campo11,
				0::numeric as campo13,
				asset.date_at as campo15,
				...
				and asset.date_at <= '%s' and (asset.f_baja is null or asset.f_baja > '%s'))T
