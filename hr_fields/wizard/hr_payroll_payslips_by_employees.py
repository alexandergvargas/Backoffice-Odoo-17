...
class HrPayslipEmployees(models.TransientModel):
	...
	structure_id = fields.Many2one(domain=lambda self:[('company_id', '=', self.env.company.id)], default=lambda self: self.get_structure_id())
	...
	def get_structure_id(self):
		...
		return self.env['hr.payroll.structure'].search([('name', '=', 'BASE'),('active', '=',True),('company_id', '=', self.env.company.id)],limit=1).id
		...
		for wizard in self:
			...
			if wizard.type_id:
				domain = expression.AND([
                    domain,
                    [('contract_ids.structure_type_id', '=', self.type_id.id)]
                ])
				# print("domain",domain)
	...
	def compute_sheet(self):
		...
		#Evite que un payslip_run tenga múltiples recibos de pago para el mismo empleado
		...
		# print("payslips_vals",payslips_vals)
