...
class HrPayslipRun(models.Model):
	...
	periodo_id = fields.Many2one('hr.period',string=u'Periodo',required=True, readonly=True, default=lambda self:self.get_period().id)
	...
	def export_plame(self):
		...
		sql = """
			...
			left join hr_type_document htd on htd.id = he.type_document_id
			where  hpr.id = {payslip_run_id}
			and CASE WHEN hm.name in ('ONP','SIN REGIMEN') THEN sr.sunat_code in ('0605') ELSE sr.sunat_code in ('0605','0601') END
			) a1
	...
	def export_plame_hours(self):
		...
		sql = """
			...
			he.identification_id as dni,
			sum(case when hpwd.work_entry_type_id in ({fal}) then hpwd.number_of_days else 0 end) as fal,
			sum(case when hpwd.work_entry_type_id in ({hext}) then hpwd.number_of_hours else 0 end) as hext,
			sum(case when hpwd.work_entry_type_id in ({dvac}) then hpwd.number_of_days else 0 end) as dvac,
			...
			and hpwd.work_entry_type_id in ({fal},{hext},{dvac})
	...
	def export_plame_other_conditions(self):
		...
		first = datetime.strftime(self.date_end, '%Y-%m-%d')[:4]
		second = datetime.strftime(self.date_end, '%Y-%m-%d')[5:7]
		doc_name = '%s0601%s%s%s.toc' % (MainParameter.dir_create_file, first, second, self.company_id.vat)
		...
		sql = """
						...
						htd.sunat_code as doc_type,
						he.identification_id as dni,
						CASE WHEN he.condition = 'not_domiciled' THEN '2' ELSE '1' END AS condition,
						hpl.total as amount
						from hr_payslip_run hpr
						inner join hr_payslip hp on hpr.id = hp.payslip_run_id
						inner join hr_payslip_line hpl on hp.id = hpl.slip_id
		...
		f.close()
		f = open(doc_name, 'rb')
		return self.env['popup.it'].get_file('0601%s%s%s.toc' % (first, second, self.company_id.vat),base64.encodebytes(b''.join(f.readlines())))
