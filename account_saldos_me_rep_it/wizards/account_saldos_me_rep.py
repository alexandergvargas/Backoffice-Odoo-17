...
class AccountSaldosMeRep(models.TransientModel):
	...
	def _get_sql(self):
		sql = """SELECT row_number() OVER () AS id, T.* FROM (select *, case when saldo_me <> 0 then saldo/saldo_me end as tc from get_saldos_me_rep('%s','%s',%d))T
	...
	def get_report(self):
		self.env.cr.execute("""
			DROP VIEW IF EXISTS account_saldos_me_book CASCADE;
	...
	def get_excel(self):
		...
		HEADERS = ['CUENTA','DENOMINACION','MONEDA','DEBE MN','HABER MN','SALDO MN','DEBE ME','HABER ME','SALDO ME','TC CIERRE']
		...
		for line in self.env['account.saldos.me.book'].search([]):
			...
			worksheet.write(x,8,line.tc if line.tc else '0.00',formats['numbercuatro'])
			x += 1

		widths = [12,25,12,15,15,15,15,15,15,12]
