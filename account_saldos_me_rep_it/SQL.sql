...
select 
  a2.code as cuenta,
  (a2.name->>'es_PE'::character varying)::character varying AS denominacion,
  coalesce(rc.name,'PEN') as moneda,
  sum(a1.debit) as debe,
  sum(a1.credit) as haber, 
  sum(a1.debit-a1.credit) as saldo,
...
where a2.currency_id is not null and a1.company_id=$3 and ( periodo_de_fecha(a3.date,a3.is_opening_close) between $1::integer and $2::integer) and a3.state='posted'
