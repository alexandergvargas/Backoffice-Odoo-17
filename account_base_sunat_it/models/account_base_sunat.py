...
class AccountBaseSunat(models.Model):
	...
	def _get_sql(self,type,period_id,company_id,x_sire=False,honorary_type_date='payment_date'):
		...
		if type == 3:
			...
			sql = """SELECT T.* FROM (SELECT 
				...
				NULL AS campo36
				FROM get_ventas_1_1('{date_start}','{date_end}',{company_id}) vst_v
				...
				am.campo_34_sale AS campo35,
				NULL AS campo36
				FROM get_ventas_1_1('2000/01/01','{before_date}',{company_id}) vst_v
				LEFT JOIN account_move am ON am.id = vst_v.am_id
		...
		if type in (4,5):
			...
			sql = """ SELECT T.* FROM (SELECT 
					...
					END AS campo8,
					CASE
						WHEN vst_d.doc_partner is not null and vst_d.doc_partner <> '' THEN REGEXP_REPLACE(vst_d.doc_partner, '[^a-zA-Z0-9]', '', 'g') 
						ELSE '0'
					END AS campo9,
					CASE
					...
					CASE
						...
						WHEN vst_d.nro_comprobante is not null and position('-' in vst_d.nro_comprobante::text) <> 0 AND split_part(vst_d.nro_comprobante, '-', 1) <> '' THEN LEFT(REGEXP_REPLACE(split_part(vst_d.nro_comprobante, '-', 1), '[^a-zA-Z0-9-]', '', 'g'), 20)
						ELSE ' '
					END AS campo11,
					CASE
		...
		if type == 6:
			#SQL Plan Contable
			code_sunat = self.env.company.l10n_pe_chart_of_accounts
			if not code_sunat:
				raise UserError(u"Debe configurar el Tipo de catalogo de cuentas PLE ubicado en Ajustes de Contabilidad para su Compañía")

			sql = """
					SELECT
					to_char(ap.date_end,'yyyymmdd') AS campo1,
					replace(T1.cuenta,'.','') AS campo2,
					LEFT(aa.name->>'es_PE',100) AS campo3,
		...
		if type == 7:
			...
			sql = """select 
				...
				tt.is_not_home,
				tt.c_d_imp,
				ec1.internal_code as honorary_type
		...
		if type == 8:
			...
			sql = """
					...
					WHERE left(cuenta,3) in ({cash_account_prefix})
			""".format(
					date_start = period_id.date_start.strftime('%Y/%m/%d'),
					date_end = period_id.date_end.strftime('%Y/%m/%d'),
					company_id = company_id,
		...
		if type == 9:
			...
			sql = """
				...
				WHERE left(cuenta,3) in ({bank_account_prefix})
			""".format(
					date_start = period_id.date_start.strftime('%Y/%m/%d'),
					date_end = period_id.date_end.strftime('%Y/%m/%d'),
					company_id = company_id,
	...
	def pdf_get_sql_vst_mayor(self,date_start,date_end,company_id):
		sql = """
			SELECT gmd.cuenta, aa.name->>'es_PE' AS name_cuenta,
	...
	def pdf_get_sql_vst_banco(self,date_start,date_end,company_id):
		sql = """
			...
			aa.account_number,
			gb.cuenta,
			aa.name->>'es_PE' as nombre_cuenta,
			gb.libro,
			gb.voucher,
	...
	def pdf_get_sql_vst_10_caja_bancos(self,period,company_id):
		sql = """
			...
			END BETWEEN %s AND %s)
			AND am.state::text = 'posted'::text  AND aml.account_id IS NOT NULL
			AND am.company_id = %d
	...
	def get_sql_pdb_purchase(self,date_start,date_end,company_id):
		sql = """
				...
				END AS field22,
				CASE
					WHEN am.linked_to_detractions = TRUE THEN am.l10n_pe_detraction_number
				END AS field23,
				CASE
					WHEN am.campo_33_purchase = TRUE THEN '1'
					ELSE '0'
	...
	def get_sql_pdb_payment(self,date_start,date_end,company_id):
		sql = """
				...
				WHERE a2.type IN ('bank','cash') AND a3.account_type='liquidity_payable' AND a1.debe IS NOT null
