	...
	def do_invoice(self):
		...
		if register:
			if register.move_id_document:
				...
				register.move_id_document.vou_number = "/"
		...
		others_document_type = self.env['account.main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).others_document_type
		destination_journal = self.env['account.main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).destination_journal

		if not others_document_type:
			raise UserError(u'No existe Tipo de Documento Otros configurado en Parametros Generales de Contabilidad para su Compañía.')
		...
		for elemnt in obj:
			...
			vals = (0,0,{
				...
				'debit': abs(elemnt[12]) if elemnt[12] < 0 else 0,
				'credit': elemnt[12] if elemnt[12] > 0 else 0,
				'amount_currency': 0,
				'currency_id': acc_ob.currency_id.id if acc_ob.currency_id else self.company_id.currency_id.id,
				'type_document_id': elemnt[8],
				'nro_comp': elemnt[3],
				'tc':1,
				'company_id': self.company_id.id,
			})
			sum_credit+= elemnt[12] if elemnt[12] > 0 else 0
			sum_debit+= abs(elemnt[12]) if elemnt[12] < 0 else 0
			lineas.append(vals)
		...
		if sum_credit > 0:
			vals_loss = (0,0,{
					...
					'debit': sum_credit,
					'credit': 0,
					'amount_currency': sum_credit,
					'currency_id': self.company_id.currency_id.id,
					'type_document_id': others_document_type.id,
	...
	def do_invoice_resumen(self):
		...
		for lin in res:
			...
			vals = (0,0,{
				...
				'amount_currency': lin['diferencia']*-1,
