...
class ImportInvoiceIt(models.Model):
	...
	account_opt = fields.Selection(
     	[
          	('default', 'Usar Cuenta Configurada en Producto'), 
           	('custom', 'Usar Cuenta de Excel')
        ], string='Cuenta de',default='default', required=True)
	type_import = fields.Selection(
     	[
        	('out_invoice', 'Cliente'),
         	('in_invoice', 'Proveedor'),
          	('out_refund',u'Nota Crédito Cliente'),
           	('in_refund',u'Nota Crédito Proveedor')
        ], string='Tipo', required=True, default='in_invoice')
	sequence_opt = fields.Selection(
     	[
        	('custom', 'Usar secuencia de Excel'), 
         	('system', 'Usar secuencia por defecto del Sistema')
        ], string='Secuencia de',default='custom')
	stage = fields.Selection(
		[
      		('draft', 'Importar Factura en Borrador'),
        	('confirm', u'Validar Factura automáticamente')
	...
	import_prod_option = fields.Selection(
     	[
            ...
            ('barcode', 'Barcode')
        ],string='Importar Producto por',default='name')
	...
	def make_invoice(self, values):
		...
		if invoice_search:
			...
			if invoice_search.nro_comp != str(values.get('nro_comprobante')):
			...
			value_inv_arr = {
				...
				'nro_comp' : str(values.get('nro_comprobante')),
				'glosa': str(values.get('glosa')),
				'company_id' : self.company_id.id,
				'type_op_det': values.get('tipo_ope'),
				'l10n_pe_detraction_date': values.get('fecha_detrac'),
				'l10n_pe_detraction_number': values.get('nro_comp_detrac'),
	...
	def make_invoice_line(self, values, inv_id):
		...
		if self.account_opt == 'default':
			if inv_id.move_type == 'out_invoice':
				...
				if product_id and product_id.property_account_income_id:
					...
					account = int(account.split(",")[1])

					account = self.env['account.account'].search([('id','=',account)],limit=1)
		...
		if values.get("analytic_distribution"):
			analytic_distribution = json.loads(self.find_analytic_account(values.get("analytic_distribution")))
			...
			if row_vali <= 0:
				...
				if str(valid[1]):
					...
					if not partner_search:
						partner.append(vat)
				if str(valid[2]):
					currency_search = self.env['res.currency'].search([('name', '=', str(valid[2]))],limit=1)
					if not currency_search:
						currency.append(str(valid[2]))
				if str(valid[3]):
					product_obj = self.env['product.template']
					if self.import_prod_option == 'barcode':
						product_search = product_obj.search([('barcode',  '=',str(valid[3]))],limit=1)
					elif self.import_prod_option == 'code':
						product_search = product_obj.search([('default_code', '=',str(valid[3]))],limit=1)
					else:
						product_search = product_obj.search([('name', '=',str(valid[3]))],limit=1)
					if not product_search:
						product.append(str(valid[3]))
				if self.account_opt =='custom':
					aux = 0
					if str(valid[4]):
						account_search = self.env['account.account'].search([('code', '=', str(valid[4])),('company_id','=',self.company_id.id)],limit=1)
						if not account_search:
							account.append(str(valid[4]))
				if str(valid[6-aux]):
					uom_search = self.env['uom.uom'].search([('name', '=', str(valid[6-aux]))],limit=1)
					if not uom_search:
						uom.append(str(valid[6-aux]))						
				if str(valid[10-aux]):
					sale_person_search = self.env['res.users'].search([('name', '=', str(valid[10-aux]))],limit=1)
					if not sale_person_search:
						sale_person.append(str(valid[10-aux]))
				if str(valid[11-aux]):
					tax_search = self.env['account.tax'].search([('name', '=', str(valid[11-aux])),('company_id','=',self.company_id.id)],limit=1)
					if not tax_search:
						tax.append(str(valid[11-aux]))
				if str(valid[14-aux]):
					td_search = self.env['l10n_latam.document.type'].search([('code', '=', str(valid[14-aux]))],limit=1)
					if not td_search:
						td.append(str(valid[14-aux]))
				
		if partner or product or currency or account or uom or sale_person or tax or td or cc:
			data = {
				'PARTNER': {'headers': [u'NRO PARTNER'], 'lines': partner},
				'MONEDA': {'headers': [u'MONEDA'], 'lines': currency},
				'PRODUCTO': {'headers': [u'PRODUCTO'], 'lines': product},
				'CUENTA': {'headers': [u'CUENTA'], 'lines': account},
				'UND MEDIDA': {'headers': [u'UNIDAD MEDIDA'], 'lines': uom},
				'VENDEDOR': {'headers': [u'VENDEDOR'], 'lines': sale_person},
				'IMPUESTO': {'headers': [u'IMPUESTO'], 'lines': tax},
				'TD': {'headers': [u'TIPO DOCUMENTO'], 'lines': td},
			}
			return self.get_excel_not_fields(data)
		else:
			self.action_validate()

	def get_excel_not_fields(self, data):
		import io
		...
		for sheet_name, sheet_data in data.items():
			...
			column_widths = sheet_data.get('column_widths', [15])
			...
			for line in sheet_data['lines']:
				...
				x += 1
			worksheet = ReportBase.resize_cells(worksheet, column_widths)

		workbook.close()

		f = open(direccion + namefile, 'rb')
		return self.env['popup.it'].get_file(u'Datos no encontrados.xlsx', base64.encodebytes(b''.join(f.readlines())))
	...
	def find_type_document(self,name):
		type_document_search = self.env['l10n_latam.document.type'].search([
      		('code','=',name)
        ],limit=1)
