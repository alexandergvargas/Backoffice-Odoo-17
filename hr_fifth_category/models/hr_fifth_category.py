...
class HrFifthCategory(models.Model):
	...
	line_ids = fields.One2many('hr.fifth.category.line', 'fifth_category_id', string='Afectos a Quinta')
	line_excluidos_ids = fields.One2many('hr.fifth.category.line.excluidos', 'fifth_category_id', string='Excluidos de Quinta')
...
class HrFifthCategoryLine(models.Model):
	...
	def get_past_months_ret(self, slip, date_from):
		...
		sr_quinta_id = self.env['hr.salary.rule'].search(
			[('company_id', '=', self.env.company.id), ('code', '=', 'QUINTA')], limit=1)
	...
	def compute_fifth_line(self):
		...
		for record in self:
			...
			grat_july_proy = record.monthly_rem * (1 + (Contract.social_insurance_id.percent)/100) if MainParameter.compute_proy_planilla else Contract.grat_july_proyected
			record.grat_july = (grat_july.total_grat+grat_july.bonus_essalud) if month >= 7 and grat_july else grat_july_proy
			grat_december_proy = record.monthly_rem * (1 + (Contract.social_insurance_id.percent)/100) if MainParameter.compute_proy_planilla else Contract.grat_december_proyected
