...
class AccountBankRep(models.TransientModel):
	...
	def get_excel(self):
		...
		x=0
		if self.show_header:
			worksheet.merge_range(x,0,x,2, self.company_id.partner_id.name, formats['especial2'])
			x+=1
			worksheet.write(x,0, self.company_id.partner_id.vat, formats['especial2'])
			x+=1
			worksheet.merge_range(x,0,x,12, "LIBRO AUXILIAR DE CAJA Y BANCOS", formats['especial5'] )
			x+=1
			worksheet.merge_range(x,0,x,12, "(DEL "+ str(self.period_start.date_start) +" AL " + str(self.period_end.date_end) +")", formats['especial5'])
			x+=2
			worksheet.write(x,0,"Cuenta Bancaria/Caja:",formats['especial2'])
			worksheet.write(x,2,self.account_id.name,formats['especial2'])
			x+=1
			worksheet.write(x,0,"Moneda:",formats['especial2'])
			worksheet.write(x,2,self.account_id.currency_id.name if self.account_id.currency_id.name else '',formats['especial2'])
			x+=1
