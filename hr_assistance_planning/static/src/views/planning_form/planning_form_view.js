...
import { AddressRecurrencyConfirmationDialog } from "@hr_assistance_planning/components/address_recurrency_confirmation_dialog/address_recurrency_confirmation_dialog";
...
export class PlanningFormController extends FormController {
    ...
    async onRecordSaved(record, changes) {
        if ("repeat" in changes && record.data["repeat"]) {
            const message = _t("Los turnos recurrentes se han creado con éxito.");
    ...
    async beforeExecuteActionButton(clickParams) {
            ...
            const canProceed = await new Promise((resolve) => {
                if (shift.data.recurrency_id) {
                    ...
                    this.dialogService.add(ConfirmationDialog, {
                        body: _t("¿Estás seguro de que quieres eliminar este turno?"),
        ...
        //     const message = _t("Este turno se guardó correctamente como plantilla.");
        //     this.notification.add(
