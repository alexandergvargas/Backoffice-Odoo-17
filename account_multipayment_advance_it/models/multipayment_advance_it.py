...
class MultipaymentAdvanceIt(models.Model):
	...
	@api.depends('company_id','journal_id','asiento_id','asiento_id.line_ids','lines_ids.is_account_cash','state')
	...
	def calculate_line(self):
		...
		main = self.env['account.main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).vario_partner_id
		val = {
			'main_id': self.id,
			'account_id': account_id,
			'currency_id': self.journal_id.currency_id.id if self.journal_id.currency_id else self.journal_id.company_id.currency_id.id,
			'importe_divisa': (amount/self.tc)*-1 if self.journal_id.currency_id else amount*-1,
			'debe': debe,
			'haber': haber,
			'is_account_cash' : True,
			'partner_id': self.invoice_ids.partner_id.id if len(self.invoice_ids) == 1 else main.id,
			'type_document_id': self.env['l10n_latam.document.type'].search([('code','=','00')],limit=1).id,
		}
	...
	def cancelar(self):
		if self.asiento_id.id:
			#PARA PODER ELIMINAR EL ASIENTO BLOQUEADO
			self.asiento_id.multipayment_advance_id = False
			account = self.asiento_id
