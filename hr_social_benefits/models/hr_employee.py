...
class HrEmployee(models.Model):
	...
	def get_pdf_liquidation(self):
		...
		data = [
				...
				[Paragraph('<strong>Motivo</strong>', style_left_tab), ':', Paragraph(Last_Contract.situation_reason_id.name.capitalize() or '', style_left)],
		...
		ExtraLine=self.env['hr.liquidation.extra_concepts'].search([('liquidation_id', '=', Liquidation.id),('employee_id', '=',self.id)],limit=1)
		...
		for inc in In_Concepts:
			in_data.append([Paragraph('<strong>{0}</strong>'.format(inc.name_input_id.name.capitalize()), style_left_tab), ':', Paragraph('{:,.2f}'.format(inc.amount) or '0.00', style_left)])
		data += in_data
		data += [
				[''],
		...
		for ouc in Out_Concepts:
			out_data.append([Paragraph('<strong>{0}</strong>'.format(ouc.name_input_id.name.capitalize()), style_left_tab), ':', Paragraph('{:,.2f}'.format(ouc.amount) or '0.00', style_left)])
		data += out_data
		data += [
				[''],
	...
	def get_pdf_liquidation_v2(self):
		...
		data = [
				...
				[Paragraph('Basico', style_left),Paragraph(': S/ {:,.2f}'.format(Last_Contract.wage or 0.00), style_left),
				 Paragraph('Periodo Liquidar', style_left),Paragraph(': %s' % Liquidation.payslip_run_id.periodo_id.name or '', style_left)],
				[Paragraph('Motivo de Cese', style_left),Paragraph(': %s' % Last_Contract.situation_reason_id.name.capitalize() or '', style_left),
		...
		if In_Concepts:
			...
			for inc in In_Concepts:
				in_data.append([Paragraph('{0}'.format(inc.name_input_id.name.capitalize()), style_left_tab_ter),'','=',
		...
		if Out_Concepts:
			...
			for ouc in Out_Concepts:
				out_data.append([Paragraph('{0}'.format(ouc.name_input_id.name.capitalize()), style_left_tab_ter),'','=',
