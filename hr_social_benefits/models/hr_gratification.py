...
class HrGratification(models.Model):
	...
	line_ids = fields.One2many('hr.gratification.line', 'gratification_id', string='Calculo de Gratificacion')
	...
	def action_template_wizard(self):
		for i in self:
			return {
				'name': 'Importar/Actualizar Gratificacion',
				'type': 'ir.actions.act_window',
				'view_mode': 'form',
				'res_model': 'import.template.bbss',
				'context': {'default_gratification_id': i.id},
				'target': 'new',
			}
...
class HrGratificationLine(models.Model):
	...
	gratification_line_ids = fields.One2many('hr.gratification.line.detalle', 'gratification_line_id')
		...
		for payslip in self:
			...
			try:
				self.env['mail.mail'].sudo().create({
					'subject': 'Boleta: %s %s' % (dict(payslip.gratification_id._fields['type'].selection).get(payslip.gratification_id.type),payslip.gratification_id.fiscal_year_id.name),
					'body_html':"""Estimado (a) %s,<br/>
									Estamos adjuntando su boleta de %s %s,<br/>
									<strong>Nota: Para abrir su boleta es necesario colocar su dni como clave y tambien confirme su recepcion</strong>
									<div style='text-align: center; padding: 16px 0px 16px 0px;'>
										<a href='%s/grati_line/%d' style='padding: 5px 10px; color: #FFFFFF; text-decoration: none; background-color: #875A7B; border: 1px solid #875A7B; border-radius: 3px'>
											Confirmar Recepcion
										</a>
									</div>""" % (Employee.name,
	...
	# Generacion de calculo de gratificacion
	def _get_sql_salary_grat(self):
		MainParameter = self.env['hr.main.parameter'].get_main_parameter()
		MainParameter.check_voucher_values()
		...
		date_calculate = self.gratification_id.payslip_run_id.date_start
		sql = """
						select      
						T.employee_id,
						T.periodo,
			...
			date_from="%s/%s/01" % (
				(date_calculate - relativedelta(months=6)).year, (date_calculate - relativedelta(months=6)).month),
	...
	def view_detail_grat(self):
		...
		self.env.cr.execute(self._get_sql_salary_grat())
		...
		for res in record:
			...
			self.env['hr.gratification.line.detalle'].create(data)
...
class HrGratificationLineDetalle(models.Model):
	...
	_description = 'hr gratification line detalle'

	gratification_line_id = fields.Many2one('hr.gratification.line', ondelete='cascade')
