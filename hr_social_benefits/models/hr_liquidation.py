...
class HrLiquidation(models.Model):
    ...
    with_bonus = fields.Boolean(string='Bono Extraordinario', default=False)
    months_and_days = fields.Boolean(string='Calcular Dias Grati.', default=False)
    ...
    gratification_type = fields.Selection([('07', 'Gratificacion Fiestas Patrias'),
                                           ('12', 'Gratificacion Navidad')], string='Tipo Gratificacion', required=True)
    ...
    gratification_line_ids = fields.One2many('hr.gratification.line', 'liquidation_id', string='Calculo de Gratificaciones Truncas')
    cts_line_ids = fields.One2many('hr.cts.line', 'liquidation_id', string='Calculo de CTS Trunca')
    vacation_line_ids = fields.One2many('hr.liquidation.vacation.line', 'liquidation_id', string='Calculo de Vacaciones Truncas')
    ...
    liq_ext_concept_ids = fields.One2many('hr.liquidation.extra_concepts', 'liquidation_id',string='Otros Conceptos')
        ...
        for Employee in Employees:
            ...
            if admission_date <= MonthSlip.date_from:
                ...
                for Lot in Lots:
                    ...
                    WorkingWD =	sum(WorkedDays.filtered(lambda line: line.work_entry_type_id in MainParameter.working_wd_ids).mapped('number_of_days'))
                    if WorkingWD >= 30:
                        months += 1
                    else:
                        days += WorkingWD

                    LackWD = sum(WorkedDays.filtered(lambda line: line.work_entry_type_id in MainParameter.lack_wd_ids).mapped('number_of_days'))
                    lacks += LackWD
                # print("lacks",lacks)
                # print("meses",months)
                # print("days",days)
                ...
                membership = MonthSlip.membership_id
                onp = afp_jub = afp_si = afp_mixed_com = afp_fixed_com = 0
                if membership.is_afp:
                    afp_jub = ReportBase.custom_round(MonthSlip.retirement_fund/100 * total_vacation, 2)
                    if total_vacation < MonthSlip.insurable_remuneration:
                        afp_si = ReportBase.custom_round(MonthSlip.prima_insurance / 100 * total_vacation, 2)
                    else:
                        afp_si = ReportBase.custom_round(MonthSlip.prima_insurance / 100 * MonthSlip.insurable_remuneration, 2)

                    if MonthSlip.commision_type == 'mixed':
                        afp_mixed_com = ReportBase.custom_round(MonthSlip.mixed_commision/100 * total_vacation, 2)
                        afp_fixed_com =0
                    elif MonthSlip.commision_type == 'flow':
                        afp_fixed_com = ReportBase.custom_round(MonthSlip.fixed_commision /100 * total_vacation, 2)
                        afp_mixed_com =0
                else:
                    onp = ReportBase.custom_round(MonthSlip.retirement_fund/100 * total_vacation, 2)
...
class HrLiquidationVacationLine(models.Model):
    ...
    def compute_vacation_line(self):
        ...
        MonthLot = self.liquidation_id.payslip_run_id

        for record in self:
            MonthSlip = MonthLot.slip_ids.filtered(lambda slip: slip.employee_id == record.employee_id)
            ...
            onp = afp_jub = afp_si = afp_mixed_com= afp_fixed_com= 0
            if membership.is_afp:
                afp_jub = ReportBase.custom_round(MonthSlip.retirement_fund/100 * record.total_vacation, 2)
                if record.total_vacation >= MonthSlip.insurable_remuneration:
                    afp_si = ReportBase.custom_round(MonthSlip.prima_insurance/100 * MonthSlip.insurable_remuneration, 2)
                else:
                    afp_si = ReportBase.custom_round(MonthSlip.prima_insurance/100 * record.total_vacation, 2)

                if MonthSlip.commision_type == 'mixed':
                    afp_mixed_com = ReportBase.custom_round(MonthSlip.mixed_commision/100 * record.total_vacation, 2)
                    afp_fixed_com =0
                elif MonthSlip.commision_type == 'flow':
                    afp_fixed_com = ReportBase.custom_round(MonthSlip.fixed_commision /100 * record.total_vacation, 2)
                    afp_mixed_com =0
            else:
                onp = ReportBase.custom_round(MonthSlip.retirement_fund/100 * record.total_vacation, 2)
...
class HrLiquidationExtraConcepts(models.Model):
    ...
    conceptos_lines = fields.One2many('hr.extra.concept.line','extra_concept_id',string='Conceptos Extras')
    ...
    def get_concepts_view(self):
        view_id = self.env.ref('hr_social_benefits.hr_extra_concept_form', False)
        return {'name':'Conceptos Adicionales',
                'type':'ir.actions.act_window',
    ...
    def add_concept(self):
        ...
        if self.conceptos_lines:
            ...
            self.env['hr.liquidation.extra_concepts'].browse(self.id).write({'income':In_Concepts,'expenses':Out_Concepts})
        else:
            self.env['hr.liquidation.extra_concepts'].browse(self.id).write({'income':0,'expenses':0})
        return self.env['hr.liquidation'].browse(self.liquidation_id.id)
...
class HrExtraConceptLine(models.Model):
    ...
    extra_concept_id = fields.Many2one('hr.liquidation.extra_concepts', 'Otros Conceptos', ondelete='cascade')
