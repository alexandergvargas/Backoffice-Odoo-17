...
class HrMainParameter(models.Model):
	...
	bonus_sr_ids = fields.Many2many('hr.salary.rule', 'sr_bonus_main_parameter_rel', 'main_parameter_id', 'sr_id', string='R. S. Bonificaciones Regulares')
	commission_sr_ids = fields.Many2many('hr.salary.rule', 'sr_commission_main_parameter_rel', 'main_parameter_id', 'sr_id', string='R. S. Comisiones')
	...
	# lack_wd_id = fields.Many2one('hr.work.entry.type', string='Worked Day Faltas')
	lack_wd_ids = fields.Many2many('hr.work.entry.type', 'sr_lack_main_parameter_rel', 'main_parameter_id', 'sr_id', string='Worked Days Faltas')
	working_wd_ids = fields.Many2many('hr.work.entry.type', 'sr_working_main_parameter_rel', 'main_parameter_id', 'sr_id', string='Worked Days Dias Laborados')
	...
	medical_rest_wd_ids = fields.Many2many('hr.work.entry.type', 'wd_medical_rest_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Descanso Medico')
	...
	def calculate_excess_medical_rest(self, year, Employee, cts_year=False):
		...
		MedicalRestWD =	sum(WorkedDays.filtered(lambda line: line.work_entry_type_id in MainParameter.medical_rest_wd_ids).mapped('number_of_days'))
		if MedicalRestWD >= 60:
