	...
	def make_vacation_rest(self):
		...
		if self.type_show == 'pantalla':
			if self.showall:
				...
				domain = [('company_id', '=', self.env.company.id), ('is_saldo_final', '=', True),
						  ('employee_id', 'in', self.employees_ids.ids)]
			c = {
				...
				'view_type': 'form',
				'view_mode': 'tree',
				'views': [(self.env.ref('hr_vacations.hr_vacation_rest_tree_resumen').id, 'tree')],
				'search_view_id': [self.env.ref('hr_vacations.hr_vacation_rest_search').id, 'search'],
				'domain': domain
			}
			# print(c)
			return c
