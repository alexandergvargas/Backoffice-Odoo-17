...
class HrVacationRest(models.Model):
	...
	def get_vacation_employee(self,employee,show_all):
		...
		for employee in employes:
			last_contract = self.env['hr.contract'].search([('employee_id','=',employee.id),
						('labor_regime','in',['general','small','micro']),('state', 'in', ['open'])])
			...
			for vaca in vacas_sorted:
				if saldo_dias==0:
					if vaca.internal_motive=='normal':
						...
						if len(vacas_sorted) == 1:
							vaca.is_saldo_final = True
					elif vaca.internal_motive=='rest':
	...
	def view_detail(self):
		return {
			...
			'view_mode': 'tree',
			'view_type': 'form',
			'views': [(self.env.ref('hr_vacations.hr_vacation_rest_tree').id, 'tree')],
			'target': '_blank',
		}