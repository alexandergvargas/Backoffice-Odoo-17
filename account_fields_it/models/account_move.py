...
class AccountMove(models.Model):
	...
	l10n_latam_document_number = fields.Char(compute='_compute_l10n_latam_document_number')
	vou_number = fields.Char(string='Voucher',default='/',copy=False)

	@api.depends('nro_comp')
	def _compute_l10n_latam_document_number(self):
		for rec in self:
			rec.l10n_latam_document_number = rec.nro_comp
	...
	def _default_invoice_date(self):
		self.invoice_date = fields.Date.context_today(self) if self.move_type != 'entry' else None
	...
	tc_per = fields.Boolean(string='Usar Tc Personalizado',copy=False,default=False)
	partner_vat = fields.Char(related='partner_id.vat',string='RUC')
	partner_l10n_pe_vat_code = fields.Char(related='partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code',string='TP')
	l10n_latam_document_type_id = fields.Many2one(change_default=True)

	#PLE COSTOS DE PRODUCCION
	code_warehouse_type = fields.Char(string=u'Tipo Almacén',size=2)

	@api.returns('self', lambda value: value.id)
	def copy(self, default=None):
		copied_am = super().copy(default)
		copied_am.l10n_latam_document_type_id = None
		copied_am.nro_comp = None
...
class AccountMoveLine(models.Model):
	...
	vou_number = fields.Char(string=_('Asiento contable'), related='move_id.vou_number')