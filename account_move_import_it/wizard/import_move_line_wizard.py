...
class ImportMoveLineWizard(models.TransientModel):
	...
	document_file = fields.Binary(string='Excel', help="El archivo Excel debe ir con la cabecera: account_id, debit, credit, currency_id, amount_currency, tc, partner_id, type_document_id, nro_comp, date_maturity, name, analytic_distribution, amount_tax, tag_ids, invoice_date_it, cta_cte")
	...
	def create_journal_entry_line(self,values):
		...
		if currency.name != 'PEN':
			vals = (0,0,{
				...
				'analytic_distribution': analytic_distribution if analytic_distribution else None,
	...
	def find_type_document(self,code):
		catalog_payment_search = self.env['l10n_latam.document.type'].search([('internal_code', '=', str(code))],limit=1)
