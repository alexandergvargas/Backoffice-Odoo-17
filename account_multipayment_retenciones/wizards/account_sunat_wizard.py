...
class AccountSunatWizard(models.TransientModel):
	...
	def get_txt_retenciones(self):
		...
		if not retention_account_id:
			raise UserError(u'Debe configurar su cuenta de Retenciones en Parametros Principales de Contabilidad para su Compañía.')
		...
		sql_query = self._get_sql_txt_retenciones(retention_journal_id,retention_account_id)
		ReportBase = self.env['report.base']
		res = ReportBase.get_file_sql_export(sql_query,'|')

		return self.env['popup.it'].get_file(name_doc,res)
	
	def _get_sql_txt_retenciones(self,retention_journal_id,retention_account_id):
		
		sql = """
			SELECT rp.vat as campo1,
			CASE
				WHEN split_part(mpl2.nro_comp::text, '-'::text, 2) <> ''::text THEN split_part(mpl2.nro_comp::text, '-'::text, 1)::character varying
				ELSE ''::character varying
			END AS campo2,
			CASE
				WHEN split_part(mpl2.nro_comp::text, '-'::text, 2) <> ''::text THEN split_part(mpl2.nro_comp::text, '-'::text, 2)::character varying
				ELSE split_part(mpl2.nro_comp::text, '-'::text, 1)::character varying
			END AS campo3,
			TO_CHAR(mp.payment_date :: DATE, 'dd/mm/yyyy') as campo4,
			round(mp.amount,2) as campo5,
			ec01.code as campo6,
			CASE
			...
			left join multipayment_advance_it_line2 mpl2 on mpl2.main_id = mp.id
			...
			where (mp.account_date between '%s' and '%s') and mp.company_id = %d and mp.journal_id = %d and mp.state = 'done' and mpl2.account_id = %d
		"""%(self.period_id.date_start.strftime('%Y/%m/%d'),
				self.period_id.date_end.strftime('%Y/%m/%d'),
				self.company_id.id,
				retention_journal_id.id,
				retention_account_id.id)
		return sql
