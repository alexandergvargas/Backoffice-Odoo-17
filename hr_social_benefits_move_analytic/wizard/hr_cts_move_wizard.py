...
class HrCtsMoveWizard(models.TransientModel):
	_name = 'hr.cts.move.wizard'
	_description = 'Hr Cts Move Wizard'
	...
	def generate_move(self):
		...
		cts = self.env['hr.cts'].browse(self._context.get('cts_id'))
		# if cts.state=='draft':
		# 	raise UserError('Primero tiene que exportar el calculo de la CTS')
		PR = cts.payslip_run_id
		extra_line = {}
		...
		move = self.env['account.move'].create({
			...
			'date': cts.deposit_date,
			'ref': 'CTS'+(PR.periodo_id.code).replace("-", ""),
			'glosa': 'CTS %s' %(PR.name),
			'line_ids': [(0, 0, {
					...
					'credit': line['credit'],
					'type_document_id' : MainParameter.type_doc_pla.id,
					'nro_comp' : 'CTS'+(PR.periodo_id.code).replace("-", ""),
