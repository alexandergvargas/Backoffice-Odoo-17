...
class AccountSunatRep(models.TransientModel):
	...
	sire_proposal = fields.Selection([('api','API'),('txt','TXT')],default='api',string='Actualizar desde')

	############ Ventas #########
	def get_ventas_sire_1(self):
		self.env.cr.execute("""DELETE FROM account_sunat_sire_sale_data""")
		...
		if self.type_show == 'pantalla':
			...
			for line in self.env['account.sunat.sire.sale.data'].search([]):
				...
				worksheet.write(x,3,line.fecEmision if line.fecEmision else '' ,formats['reverse_dateformat'])
				worksheet.write(x,4,line.fecVencPag if line.fecVencPag else '' ,formats['reverse_dateformat'])
				...
				worksheet.write(x,27,line.fecEmisionMod if line.fecEmisionMod else '' ,formats['reverse_dateformat'])
	...
	def get_compras_sire_1(self):
		...
		for line in data:
			...
			worksheet.write(x,3,line[4] if line[4] else '' ,formats['reverse_dateformat'])
			worksheet.write(x,4,line[5] if line[5] else '' ,formats['reverse_dateformat'])
			...
			worksheet.write(x,26,line[27] if line[27] else '' ,formats['reverse_dateformat'])
	...
	def sql_sire_1_2_sale(self,date_from,date_to,company_id,cpf=False):
		sql = """SELECT T.* FROM (SELECT 
				...
				NULL as campo40,
				NULL as campo41
				FROM get_ventas_1_1('{date_from}', '{date_to}', {company_id}) vst_v
	...
	def sql_sire_2_purchase(self,date_from,date_to,company_id):
		sql = """SELECT T.* FROM (
			...
			case when vst_c.td = '14' then TO_CHAR(vst_c.fecha_v :: DATE, 'dd/mm/yyyy') else NULL end as campo6,
			vst_c.td as campo7,
			CASE
			...
			NULL AS campo41,
			NULL AS campo42
			from get_compras_1_1('{date_from}', '{date_to}', {company_id}) vst_c
