...
class AccountMoveLine(models.Model):
	...
	def _compute_price_unit(self):
		for line in self:
			...
			line.price_unit = line.product_id.with_context(tc_per=tc_per,manual_currency_rate=manual_currency_rate)._get_tax_included_unit_price(
	...
	@api.depends('currency_id', 'company_id', 'move_id.invoice_date' ,'move_id.date', 
		'move_id.tc_per', 'move_id.manual_currency_rate')
	...
	def _prepare_reconciliation_single_partial(self, debit_values, credit_values, shadowed_aml_values=None):
		...
		if debit_values.get('record') and debit_values['record'].move_id.tc_per and debit_values['record'].move_id.manual_currency_rate:
			debit_values['manual_currency_rate'] = debit_values['record'].move_id.manual_currency_rate
		if credit_values.get('record') and credit_values['record'].move_id.tc_per and credit_values['record'].move_id.manual_currency_rate:
...
class AccountMove(models.Model):
	...
	def _compute_manual_currency_rate(self):
		for i in self:
			i.manual_currency_rate = (1/i.currency_rate) if i.currency_rate != 0 else 0
		...
		for record in self:
			if record.tc_per:
				if record.currency_rate == 0:
					raise UserError('El tipo de Cambio no puede ser 0.')

	@api.onchange('tc_per', 'currency_id')
	...
	@api.onchange('invoice_date','tc_per','l10n_latam_document_type_id')
	...
	def _onchange_currency_rate(self):
		if self.is_invoice(include_receipts=True) and self.tc_per:
	...
	def _compute_payments_widget_to_reconcile_info(self):
		for move in self:
			...
			domain = [
				...
				('nro_comp', '=', move.l10n_latam_document_number)
			]
			...
			for line in self.env['account.move.line'].search(domain):
				...
				if line.currency_id == move.currency_id:
					...
					if move.tc_per and move.manual_currency_rate:
...
class ProductProduct(models.Model):
	...
	def _get_tax_included_unit_price(self, company, currency, document_date, document_type,
		...
		tc_per = self._context.get('tc_per')
