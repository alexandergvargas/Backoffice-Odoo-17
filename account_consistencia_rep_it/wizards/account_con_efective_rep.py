...
class AccountConEfectiveRep(models.TransientModel):
	...
	def get_excel(self):
		...
		HEADERS = ['CUENTA','T FLUJO EFECTIVO','T FLUJO CAJA','INGRESO','EGRESO','BALANCE']
		...
		for line in self.env['account.con.efective.book'].search([]):
			...
			worksheet.write(x,2,line.account_cash_flow_id if line.account_cash_flow_id else '',formats['especial1'])
			worksheet.write(x,3,line.ingreso if line.ingreso else '0.00',formats['numberdos'])
			worksheet.write(x,4,line.egreso if line.egreso else '0.00',formats['numberdos'])
			worksheet.write(x,5,line.balance if line.balance else '0.00',formats['numberdos'])
		...
		worksheet.write(x,3,ingreso,formats['numbertotal'])
		worksheet.write(x,4,egreso,formats['numbertotal'])
		worksheet.write(x,5,balance,formats['numbertotal'])

		widths = [9,56,56,12,12,12]
	...
	def _get_sql(self):
		sql = """
				...
				GROUP BY vst_d.cuenta,ati.name,acf.name) T
			""".format(
