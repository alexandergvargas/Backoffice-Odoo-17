...
class bank_loans(models.Model):
	...
	loan_expense_commissions = fields.Float(u'Comisiones Gasto Préstamo',tracking=True,default=0)
	...
	loan_expense_com_account_id = fields.Many2one('account.account', string=u'Cuenta para Comisiones Gasto Préstamo',tracking=True,copy=False)
	interest_debt_account = fields.Many2one('account.account', string=u'Cuenta de Deuda Interés',tracking=True,copy=False)
	currency_id = fields.Many2one('res.currency', string='Moneda',compute="_compute_currency_id")
	tc = fields.Float(u'Tipo de Cambio',tracking=True,default=1,digits=(12,3))
	#########################################################################################
	...
	def validate_fields(self):
		for i in self:
			...
			if not i.interest_debt_account:
				raise ValidationError(u'Es necesario el campo "Cuenta de Deuda Interés"')
			if not i.loan_expense_com_account_id:
				raise ValidationError(u'Es necesario el campo "Cuenta para Comisiones Gasto Préstamo"')
	...
	def libray_data_line(self,doc):
		for i in self:
			move_lines = []
			account = [i.company_id.account_journal_payment_debit_account_id.id,i.interest_account_id.id,i.due_account_id.id,i.interest_debt_account.id,i.loan_expense_com_account_id.id]
			amount_currency = [i.cap_pres,i.inters,(i.cap_pres*-1)+(i.loan_expense_commissions*-1),i.inters*-1,i.loan_expense_commissions]
			if i.currency_id.name == 'USD':
				debit =  [i.cap_pres*i.tc,i.inters*i.tc,0,0,i.loan_expense_commissions*i.tc]
				credit =  [0,0,(i.cap_pres*i.tc)+(i.loan_expense_commissions*i.tc),i.inters*i.tc,0]
			else:
				debit =  [i.cap_pres,i.inters,0,0,i.loan_expense_commissions]
				credit =  [0,0,i.cap_pres+i.loan_expense_commissions,i.inters,0]
			for r in range(5):
				line_firt = (0,0,{
	...
	@api.depends('cap_pres','inters','loan_expense_commissions')
	def _compute_amount_total_debt(self):
		for i in self:
			i.amount_total_debt = i.cap_pres + i.inters + i.loan_expense_commissions
