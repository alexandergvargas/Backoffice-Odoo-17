...
currency_from_id integer,currency_to_id integer,amount numeric, date date, company_id integer)
    RETURNS numeric
    LANGUAGE 'sql'
    COST 100
    VOLATILE 
...
select ($3*((SELECT case when exists (select rc_to.rate from 
(SELECT name,currency_id, rate FROM res_currency_rate where company_id = $5 ORDER BY name) rc_to 
where rc_to.currency_id = currency_to_id and rc_to.name = date) then (select rc_to.rate from 
(SELECT name,currency_id, rate FROM res_currency_rate where company_id = $5 ORDER BY name) rc_to 
where rc_to.currency_id = currency_to_id and rc_to.name = date) else 1 end)/(select case when exists (select rc_from.rate from 
(SELECT name,currency_id, rate FROM res_currency_rate where company_id = $5 ORDER BY name) rc_from 
where rc_from.currency_id = currency_from_id and rc_from.name = date) then (select rc_from.rate from 
(SELECT name,currency_id, rate FROM res_currency_rate where company_id = $5 ORDER BY name) rc_from 
where rc_from.currency_id = currency_from_id and rc_from.name = date) else 1 end)))::numeric as amount
$BODY$;
------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.get_diariog(
	...
	tc numeric, importe_me numeric, glosa character varying, td_partner character varying, doc_partner character varying, partner character varying, td_sunat character varying, 
...
order by aa.code,T.account_id,T.fecha,T.move_line_id;
END;
	$BODY$
	LANGUAGE plpgsql VOLATILE
	COST 100
...
CREATE OR REPLACE FUNCTION public.get_ventas_1(
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '1'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS exp_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '2'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS venta_g_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '3'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS inaf_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '4'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS exo_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '5'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS isc_v_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '6'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS otros_v_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '7'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS igv_v_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_sale::text = '8'::text THEN (case when rc.name = 'USD' then (- aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(-aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS icbper_me
	FROM account_move_line aml
	...
	WHERE am.state::text = 'posted'::text 
	--AND aml.display_type IS NULL 
	AND aml.account_id IS NOT NULL AND (am.date::date BETWEEN $1 and $2) AND am.company_id = $3
...
END;
	...
	ROWS 1000;
--------------------------------------------------------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.get_ventas_1_1(date, date, integer) CASCADE;
...
RETURNS TABLE(periodo integer, fecha_cont date,libro character varying, voucher character varying, fecha_e date, fecha_v date,
	...
	SELECT CASE
		...
		END AS periodo,
			...
			CASE WHEN am.state = 'cancel' then '0'::character varying else lit.l10n_pe_vat_code end AS tdp,
			CASE WHEN am.state = 'cancel' THEN rp2.vat ELSE rp.vat END AS docp,
			CASE WHEN am.state = 'cancel' THEN rp2.name ELSE rp.name END AS namep,						
		   ...
		   FROM account_move am
			 ...
			 LEFT JOIN l10n_latam_document_type ec1 ON ec1.id = (case when am.state = 'posted' then am.l10n_latam_document_type_id else (select id from l10n_latam_document_type where code = '00' limit 1) end)
			 ...
			 LEFT JOIN res_currency rc ON rc.id = am.currency_id
			 LEFT JOIN account_main_parameter amp ON amp.company_id = am.company_id
			 LEFT JOIN res_partner rp2 ON rp2.id = amp.cancelation_partner
		  WHERE aj.register_sunat::text = '2'::text AND am.state::text in ('posted'::text,'cancel'::text)
...
SELECT am.id as move_id,
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '1'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS base1_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '2'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS base2_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '3'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS base3_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '4'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS cng_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '5'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS isc_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '6'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS otros_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '7'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS igv1_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '8'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS igv2_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '9'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS igv3_me,
	sum(
		CASE
	...
	sum(
		CASE
			WHEN aat.record_shop::text = '10'::text THEN (case when rc.name = 'USD' then (aml.tax_amount_me) else round(convert_currency(am.currency_id,1,(aml.tax_amount_it),(case when am.move_type = 'entry' then am.date else am.invoice_date end),am.company_id),2) end)
			ELSE 0::numeric
		END) AS icbper_me
	from account_move_line aml
	LEFT JOIN account_account_tag_account_move_line_rel rel ON rel.account_move_line_id = aml.id
...
END;
	...
	ROWS 1000;
--------------------------------------------------------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.get_compras_1_1(date, date, integer) CASCADE;
	...
	SELECT CASE
		...
		END AS periodo,
		   ...
		   FROM account_move am
			 ...
			 LEFT JOIN account_main_parameter amp ON amp.company_id = am.company_id
			 LEFT JOIN res_partner rp2 ON rp2.id = amp.cancelation_partner
		  WHERE aj.register_sunat::text = '1'::text AND am.state::text in ('posted'::text,'cancel'::text)
		  AND (am.date::date BETWEEN $1 and $2) AND am.company_id = $3
		  ORDER BY ("left"(to_char(am.date::timestamp with time zone, 'yyyymmdd'::text), 6)), aj.code, am.vou_number;
END;
	$BODY$
...
SELECT T.* FROM(SELECT 
	periodo_de_fecha(am.date,am.is_opening_close)::character varying as periodo,
	am.date as fecha,
	aj.code AS libro,
	am.vou_number AS voucher,
	a2.code AS cuenta,
  ...
  WHERE a2.check_moorage = true AND am.company_id = $3
  AND (a3.code::text || a4.code::text) IS NOT NULL AND (periodo_de_fecha(am.date,am.is_opening_close) between $1 and $2)
UNION ALL
 SELECT 
 	periodo_de_fecha(b8.date,b8.is_opening_close)::character varying as periodo,
	b8.date as fecha,
	b9.code as libro,
	b8.name as voucher,
	b3.code as cuenta,
	...
	LEFT JOIN account_move_line b2 ON b2.id = b1.move_line_id
	...
	LEFT JOIN account_move b8 ON b8.id = b2.move_id
	LEFT JOIN account_journal b9 on b9.id = b8.journal_id
  WHERE b3.check_moorage = true AND (periodo_de_fecha(b8.date,b8.is_opening_close) between $1 and $2) 
  AND b2.company_id = $3) T
...
CREATE OR REPLACE FUNCTION public.get_asiento_destino(
	IN var_periodo integer,
	IN var_company_id integer)
  RETURNS TABLE(periodo integer, glosa text,cuenta character varying, name character varying, debe numeric, haber numeric, account_id integer, deprecated boolean) AS
$BODY$
BEGIN
RETURN QUERY 
select $1::integer as periodo, ('Por el destino del Periodo ' || $1)::text as glosa, aa.code as cuenta, (aa.name->>'es_PE')::character varying, T.debe, T.haber, aa.id as account_id, aa.deprecated from (
select 
