...
class AccountBookPerceptionWizard(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in res:
			...
			worksheet.write(x,2,line['fecha_uso'] if line['fecha_uso'] else '',formats['reverse_dateformat'])
			...
			worksheet.write(x,11,line['fecha_com_per'] if line['fecha_com_per'] else '',formats['reverse_dateformat'])
			...
			if self.type == 'det':
				...
				worksheet.write(x,16,line['fecha_cp'] if line['fecha_cp'] else '',formats['reverse_dateformat'])
