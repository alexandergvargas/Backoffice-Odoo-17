...
class HrAutomateMultipayment(models.Model):
	...
	def get_amounts_total_hr(self, gratification):
		...
		if gratification:
			...
			Lines = Gratification.line_ids.filtered(lambda l: l.employee_id in self.grat_line_ids.mapped('employee_id') and l.is_txt == True)
			return sum(Lines.mapped('total'))
		else:
			amounts = []
			for slip in self.slip_ids.filtered(lambda l: l.is_txt == True):
				# if self.is_biweekly_advance:
				#	 amounts.append(self.env['report.base'].custom_round(slip.biweekly_advance, 2))
				# else:
				amounts.append(self.env['report.base'].custom_round(slip.net_wage, 2))
			return sum(amounts)
	...
	def get_bbva_hr_txt(self, gratification=False):
		...
		if gratification:
			slip_ids = self.grat_line_ids.filtered(lambda l: l.is_txt == True)
		else:
			slip_ids = self.slip_ids.filtered(lambda l: l.is_txt == True)
...
{filler}{cell_phone}{email}\r\n'.format(
		...
		for line in slip_ids:
			...
			if amount_to_pay>0:
				...
				f.write('{numcor}{p_doc_type}{p_doc_num}{apepat}{apemat}{names}{street}{phone}{platype}{codbank}{account_number}{currency_type}{amount}{motdep}'.format(
					...
					street=self.parse_to_txt(line.employee_id.user_partner_id.street if line.employee_id.user_partner_id.street else 'SIN DIRECCION', 'str', range=60,left=True),
					phone=self.parse_to_txt(line.employee_id.user_partner_id.mobile if line.employee_id.user_partner_id.mobile else 'SINTELEFON', 'str', range=10,left=True),
					platype='H',
	...
	def get_cts_lines(self):
		if self.cts_dollars:
			Lines = self.cts_line_ids.filtered(lambda l: l.employee_id.cts_bank_account_id.currency_id.name == 'USD' and l.is_txt == True)
		else:
			Lines = self.cts_line_ids.filtered(lambda l: l.employee_id.cts_bank_account_id.currency_id.name == 'PEN' and l.is_txt == True)
		return Lines
	...
	def get_bcp_cts_txt(self):
		...
		if not isinstance(BankAccount.acc_number, str):
			raise UserError("El numero de Cuenta debe ser una cadena de caracteres")

		valid_line_acc_numbers = []
		for line in self.cts_line_ids:
			if not isinstance(line.cts_account.acc_number, str):
				raise UserError("El numero de Cuenta debe ser una cadena de caracteres")
			if line.cts_dollars > 0 or line.cts_soles > 0:
				valid_line_acc_numbers.append(line.cts_account.acc_number)

		Lines = self.get_cts_lines()
		amounts_total = self.get_amounts_total_cts(Lines)

		check_sum = str(
			int(BankAccount.acc_number[3:]) +
			sum(int(acc_number[3:]) for acc_number in valid_line_acc_numbers)
		).rjust(15, '0')
		...
		for line in Lines:
			PartnerAccount = line.employee_id.cts_bank_account_id
			if line.cts_dollars > 0 or line.cts_soles > 0:
				f.write('2{payment_doc}{p_doc_type}{p_doc_num}{beneficiary_name}{beneficiary_ref}{company_ref}{currency}{amount}\r\n'.format(
					payment_doc=self.parse_to_txt(PartnerAccount.acc_number, 'str', range=20),
					p_doc_type=line.employee_id.type_document_id.bcp_code,
					p_doc_num=self.parse_to_txt(line.employee_id.identification_id, 'str', range=15),
					beneficiary_name=self.parse_to_txt(line.employee_id.name, 'str', range=75),
					...
					company_ref=self.parse_to_txt(self.env.company.name, 'str', range=20),
					currency='1001' if self.cts_dollars else '0001',
					amount=str(self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.cts_soles, 'float', range=17, left=False, decimal_point=True)) + str('1001' if self.cts_dollars else '0001') + str(self.parse_to_txt((line.wage + line.household_allowance) * 4 if self.cts_dollars else (line.wage + line.household_allowance) * 4, 'float', range=17, left=False, decimal_point=True)),
				))
...
# 		))
		f.write('0106{espacios}{date}{espacios2}{lines}{soles_total}{usd_total}MC001\r\n'.format(
			espacios=' ' * 36,
			date=self.parse_to_txt(fields.Datetime.now(), 'datetime'),
			espacios2=' ' * 9,
			lines=self.parse_to_txt(len(Lines), 'int', left=False, range=6),
			soles_total=self.parse_to_txt(amounts_total, 'float', left=False, range=15) if BankAccount.currency_id.name == 'PEN' else '0' * 15,
			usd_total=self.parse_to_txt(amounts_total, 'float', range=15) if BankAccount.currency_id.name == 'USD' else '0' * 15,
		))
		for line in Lines:
	...
	# 			))
				f.write('02{doc_type}{benef_code}{doc_number}{date_to}{charge_currency}{amount} {charge_type}{account_type}\
							{account_currency}{acc_number}{person_type}{p_doc_type}{p_doc_num}{benef_name}{currency_cts}{amount_cts}{espacios}\r\n'.format(
					doc_type=line.employee_id.type_document_id.interbank_code,
					benef_code=self.parse_to_txt(line.employee_id.identification_id, 'str', range=20),
					doc_number=' ' * 19,
					date_to=' ' * 8,
					charge_currency='01' if BankAccount.currency_id.name == 'PEN' else '10',
					amount=self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.cts_soles, 'float', left=False, range=15),
					# amount=self.parse_to_txt(amount_to_pay, 'float', left=False,range=15),
					charge_type='99' if PartnerAccount.type_of_account == '3' else '09',
					account_type=ACCOUNT_TYPE.get(PartnerAccount.type_of_account, ' ' * 3),
					account_currency=' ' * 2 if PartnerAccount.type_of_account == '3' else ('01' if BankAccount.currency_id.name == 'PEN' else '10'),
					...
					p_doc_num=self.parse_to_txt(line.employee_id.identification_id, 'str', range=15),
					benef_name=benef_name,
					currency_cts='01' if BankAccount.currency_id.name == 'PEN' else '10',
					amount_cts=self.parse_to_txt((line.wage + line.household_allowance) * 4 if self.cts_dollars else (line.wage + line.household_allowance) * 4, 'float', left=False, range=15),
					espacios=' '*186,
					# filler=' ' * 6,
					# cell_phone=self.parse_to_txt(line.employee_id.mobile_phone or '', 'str', range=40),
					# email=self.parse_to_txt(line.employee_id.work_email or '', 'str', range=140),
				))
		f.close()
		f = open(doc_name, 'rb')
	...
	def get_banbif_hr_utility_txt(self):
		...
		for line in self.slip_ids:
			...
			f.write('{numcor}{p_doc_type}{p_doc_num}{apepat}{apemat}{names}{street}{phone}{platype}{codbank}{account_number}{currency_type}{amount}{motdep}'.format(
				...
				apemat=self.parse_to_txt(line.employee_id.m_last_name, 'str', range=20,left=True),
				names=self.parse_to_txt(line.employee_id.names, 'str', range=44,left=True),
				street=self.parse_to_txt(line.employee_id.user_partner_id.street if line.employee_id.user_partner_id.street else ' ', 'str', range=60,left=True),
				phone=self.parse_to_txt(line.employee_id.user_partner_id.mobile if line.employee_id.user_partner_id.mobile else ' ', 'str', range=10,left=True),
				platype='H',
