...
class ImportAccountWizard(models.TransientModel):
	...
	type_import = fields.Selection(
     	[
        	('import','Importar Cuentas'),
         	('copy',u'Copiar a partir de una Compañía')
        ],string='Tipo', required=True,default="import")
	...
	def copy_accounts(self):
		...
		if 'create_asset' in self.env['account.account']._fields:
			self.env.cr.execute(""" 
				INSERT INTO ACCOUNT_ACCOUNT (NAME,CODE,CURRENCY_ID,ROOT_ID,DEPRECATED,ACCOUNT_TYPE,INTERNAL_TYPE,INTERNAL_GROUP,RECONCILE,NOTE,COMPANY_ID,
				...
				SELECT NAME,CODE,CURRENCY_ID,ROOT_ID,DEPRECATED,ACCOUNT_TYPE,INTERNAL_TYPE,INTERNAL_GROUP,RECONCILE,NOTE, %d AS COMPANY_ID,
				GROUP_ID,M_CLOSE,ACCOUNT_TYPE_IT_ID,ACCOUNT_TYPE_CASH_ID,PATRIMONY_ID,TYPE_ADQUISITION,CHECK_MOORAGE,
	...
	def import_file(self):
		...
		for row_no in range(sheet.nrows):
			if row_no <= 0:
				...
				formatted_value = self.validate_and_format(line[0])
				values.update( {'code' : formatted_value,
								'name' : line[1],
								'user' : line[2],
								'currency' :line[3],
								'reconcile':line[4],
								'deprecat' :line[5],
								'tipo_ef' : line[6],
								'metodo_cierre' : line[7],
								'cuenta_cierre' : line[8],
								'tipo_fe' : line[8],
								'tiene_destino' : line[10],
								'a_debe' : line[11],
								'a_haber' : line[12],
								'es_analisis' : line[13],
								'clasificacion_ht' : line[14],
								'code_sunat' : line[15],
								})
	...
	def create_chart_accounts(self,values):
		...
		if account_search:
			...
			if values.get("user") or values.get("user") != "":
				user_type = self.find_user_type(values.get('user'))
				account_search.write({'account_type': user_type})
...
# ---------------------------user-----------------
	...
	def find_user_type(self,user):
		...
		account_type=dict(self.env['account.account']._fields['account_type'].selection)
		# Agregar campos nativos del Selection
		account_type.update({
		...
		account_type={valor: clave for clave, valor in account_type.items()}
		if not account_type.get(user):
			raise UserError(f"No se encontro el Tipo de Cuenta '{user}'")
		return account_type.get(user)
# --------------------currency--------------------
...
# --------------------tag_ids--------------------
	...
	def validate_and_format(self, value):		
