...
class ResPartner(models.Model):
	...
	def _verify_document(self):
		if self.l10n_latam_identification_type_id:
			if self.l10n_latam_identification_type_id.l10n_pe_vat_code == '6' and self.company_type == 'company':
				self.street = 'Calle'
			else:
				self.street = ''
		else:
	...
	def verify_dni(self):
		...
		if not parameters:
			raise UserError(u'Debe configurar los parámetros de RUC/DNI.')
		if not self.vat:
			...
			if self.l10n_latam_identification_type_id.l10n_pe_vat_code == '6':
				...
				if parameters.query_supplier == '1':
					...
					if flag:
						...
						raise UserError(u"El RUC de %s es inválido. (%s)"%(self.name,self.vat or ''))
					...
					if len(content_ruc)==18:
						...
						self.executing_unit = u"GOBIERNO" in content_ruc[1]
					...
					self.name = (content_ruc[0].split("-")[1].replace('"','')).strip()
