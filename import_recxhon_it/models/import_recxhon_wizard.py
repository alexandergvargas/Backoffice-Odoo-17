...
class ImportRecxhonWizard(models.Model):
	...
	def import_invoices(self):
		...
		if self.document_file:
			...
			file_content = base64.decodebytes(self.document_file)
					...
					try:
						if data[3] == 'NO ANULADO':
							...
							inv_id = invoice_obj.create({
								...
								'nro_comp' : data[2] if data[2] else '',
							...
							if not tax:
								raise UserError('"%s" Tax not in your system' % tax_name)
							tax_ids.append(tax.id)

							product_uom = self.env['uom.uom'].search([('name', '=', 'Unidades')],limit=1)
							if not product_uom:
								raise UserError(' "Unidades" Product UOM category is not available.')
	...
	def find_currency(self, name):
		...
		if currency_search:
			...
			raise UserError(' "%s" Currency are not available.' % name)
	...
	def find_type_document(self,name):
		...
		if type_document_search:
			...
			raise UserError('No existe un Tipo de Comprobante con el Codigo"%s"'% name)