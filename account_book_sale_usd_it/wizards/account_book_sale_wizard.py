...
class AccountBookSaleWizard(models.TransientModel):
	_inherit = 'account.book.sale.wizard'
	...
	def get_report(self):
		...
		if self.type_show == 'pantalla':
			...
			if self.currency == 'pen':
				...
				view_id = self.env.ref('account_book_sale_usd_it.view_account_book_sale_usd_view_tree')
			return {
	...
	def get_excel(self):
		...
		for line in res:
			...
			worksheet.write(x,c+24,line['fecha_det'] if line['fecha_det'] else '',formats['reverse_dateformat'])
			worksheet.write(x,c+25,line['comp_det'] if line['comp_det'] else '',formats['especial1'])
			worksheet.write(x,c+26,line['f_doc_m'] if line['f_doc_m'] else '',formats['reverse_dateformat'])
		...
		if self.currency=='pen':
			widths = [11,14,7,11,11,12,3,7,5,10,4,12,40,11,11,11,11,11,11,11,11,11,5,12,6,12,12,16,11,9,13,71]
		else:
			widths = [11,14,7,11,11,12,3,7,5,10,4,12,40,11,11,11,11,11,11,11,11,11,5,6,12,12,16,11,9,13,71]
	...
	def get_header(self):
		HEADERS =  super(AccountBookSaleWizard,self).get_header()
	...
	def _get_sql(self):
		sql =  super(AccountBookSaleWizard,self)._get_sql()
		if self.currency == 'usd':
