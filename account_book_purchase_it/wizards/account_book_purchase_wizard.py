...
class AccountBookPurchaseWizard(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in res:
			...
			worksheet.write(x,27,line['fecha_det'] if line['fecha_det'] else '',formats['reverse_dateformat'])
			worksheet.write(x,28,line['comp_det'] if line['comp_det'] else '',formats['especial1'])
			worksheet.write(x,29,line['f_doc_m'] if line['f_doc_m'] else '',formats['reverse_dateformat'])
