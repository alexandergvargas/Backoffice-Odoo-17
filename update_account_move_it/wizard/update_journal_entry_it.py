...
class UpdateJournalEntryIt(models.TransientModel):
	...
	def update_move(self, values):
		...
		if str(values.get('asiento_nro')) == '':
			raise UserError(_('El campo "asiento_nro" no puede estar vacio.'))
		if str(values.get('journal_id')) == '':
			raise UserError(_('El campo "journal_id" no puede estar vacio.'))
		...
		if move_search:
			move_search.write({
				...
				'l10n_pe_detraction_date': values.get("fecha_det") if values.get("fecha_det") else None,
				'detraction_percent_id': detraction_percent_id,
				'l10n_pe_detraction_number': values.get("nro_comp_det") if values.get("nro_comp_det") else '',
	...
	def find_detractions_percent(self,code):
		detraciont_search = self.env['detractions.catalog.percent'].search([
      		('code','=',code)
        ],limit=1)
