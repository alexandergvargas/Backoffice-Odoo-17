...
class report_contract_employee(models.AbstractModel):
    ...
    def _conversion(self, template, code):
        ...
        if row.date_start and row.trial_date_end:
            months_prueba = (row.trial_date_end.year - row.date_start.year) * 12 + row.trial_date_end.month - row.date_start.month
        ...
        fields = {
            ...
            'direccion_empleador': row.company_id.street if row.company_id.street else "",
            'distrito': row.company_id.partner_id.state_id.name if row.company_id.partner_id.state_id.name else "",
            'provincia': row.company_id.partner_id.state_id.name if row.company_id.partner_id.state_id.name else "",
            ...
            'direccion_trabajador': row.employee_id.private_street if row.employee_id.private_street else "",
