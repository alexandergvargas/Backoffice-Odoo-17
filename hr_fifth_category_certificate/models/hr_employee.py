...
class HrEmployee(models.Model):
	...
	def get_fifth_wizard(self):
		wizard = self.env['hr.fifth.category.wizard'].create({'name': 'Certificado de Quinta'})
		return {
	...
	def get_past_rem(self, FiscalYear):
	...
	def get_past_months_ret(self, FiscalYear):
		MainParameter = self.env['hr.main.parameter'].get_main_parameter()
		MainParameter.check_fifth_values()
	...
	from (select hpl.employee_id,
	      ...
	      where extract(year from hpl.date_from) = '{0}'
	        ...
	        and hp.payslip_run_id is not null)T
	group by T.employee_id
					""".format(int(FiscalYear.name), self.id, sr_quinta_id.id,  self.env.company.id)
		self._cr.execute(sql)
		past_lines = self._cr.dictfetchall()
		ret_ant = past_lines[0]['total'] if past_lines else 0
		return ret_ant

	def get_pdf_fifth_certificate(self, fiscal_year_id, date):
		MainParameter = self.env['hr.main.parameter'].get_main_parameter()
		MainParameter.check_fifth_values()
		...
		data = [
			...
			[Paragraph('1.1. Sueldo, asignaciones, gratificaciones, bonificaciones, comisiones, etc.', style_left_tab),
			 Paragraph('S/ %d' % REQ_ROQ, style_left)],
			[Paragraph('1.2. Ingresos percibidos en otros empleadores', style_left_tab),
			 Paragraph('S/ %d' % real_other_emp_rem, style_left)],
