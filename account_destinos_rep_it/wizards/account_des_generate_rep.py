...
class AccountDesGenerateRep(models.TransientModel):
	...
	def get_report(self):
		...
		sql = """
			...
			CREATE OR REPLACE view account_des_generate_book as (SELECT row_number() OVER () AS id, a1.periodo::character varying, a1.glosa, a1.cuenta, a1.name, a1.debe, a1.haber, a1.deprecated from get_asiento_destino(%s,%d) a1)""" % (
	...
	def generate_as(self):
		...
		for elemnt in obj:
			vals = (0,0,{
				...
				'debit': elemnt[4],
				'credit': elemnt[5], 
				'amount_currency':(elemnt[4] or 0) - (elemnt[5] or 0),
