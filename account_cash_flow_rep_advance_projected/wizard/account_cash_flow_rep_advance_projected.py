...
class AccountCashFlowRepAdvanceProjected(models.TransientModel):
	_name = 'account.cash.flow.rep.advance.projected'
	...
	def get_report(self):
		...
		if self.type_show == 'pantalla':
			self.env.cr.execute("""DELETE FROM account_cash_flow_book_advance_projected WHERE user_id = %d"""%(self.env.uid))
		
			self.env.cr.execute("""
				INSERT INTO account_cash_flow_book_advance_projected (journal_id,voucher,fecha,glosa,account_id,amount,grupo,concepto,user_id) 
				("""+self._get_sql()+""")""")
			
			return {
					'name': 'Reporte de Flujo de Caja Proyectado',
					'type': 'ir.actions.act_window',
					'res_model': 'account.cash.flow.book.advance.projected',
			...
			for iterator in (day_arr if self.show_by == 'date' else perr_arr): 
				...
				if iterator == (self.date_from if self.show_by == 'date' else self.period_from_id):
					worksheet.write(x,y,self.get_saldo('am.date',False,datetime.strptime('%s/01/01'%(self.fiscal_year_id.name), '%Y/%m/%d').date(),(self.date_from if self.show_by == 'date' else self.period_from_id.date_start) - timedelta(days=1),False),numberdos_simple)
				else:
					worksheet.write_formula(x,y, '=' + xl_rowcol_to_cell(pos_saldo_final,y-1), numberdos_simple)
				x += 3

				for ingresos in self.env['account.cash.flow'].search([('grupo','=','2')]):
					worksheet.write(x,y,self.get_saldo('aml.expected_pay_date',parameters.use_counterpart_cash_flow,(iterator if self.show_by == 'date' else iterator.date_start),(iterator if self.show_by == 'date' else iterator.date_end),True,ingresos.id),numberdos_simple)
					x+= 1
				worksheet.write(x,y,self.get_saldo('aml.expected_pay_date',parameters.use_counterpart_cash_flow,(iterator if self.show_by == 'date' else iterator.date_start),(iterator if self.show_by == 'date' else iterator.date_end),True,sign='>'),numberdos_simple)
				x+=1
				...
				for egresos in self.env['account.cash.flow'].search([('grupo','=','3')]):
					worksheet.write(x,y,self.get_saldo('aml.expected_pay_date',parameters.use_counterpart_cash_flow,(iterator if self.show_by == 'date' else iterator.date_start),(iterator if self.show_by == 'date' else iterator.date_end),True,egresos.id),numberdos_simple)
					x+= 1
				worksheet.write(x,y,self.get_saldo('aml.expected_pay_date',parameters.use_counterpart_cash_flow,(iterator if self.show_by == 'date' else iterator.date_start),(iterator if self.show_by == 'date' else iterator.date_end),True,sign='<'),numberdos_simple)
				x+=1
				...
				for finan in self.env['account.cash.flow'].search([('grupo','=','4')]):
					worksheet.write(x,y,self.get_saldo('aml.expected_pay_date',parameters.use_counterpart_cash_flow,(iterator if self.show_by == 'date' else iterator.date_start),(iterator if self.show_by == 'date' else iterator.date_end),True,finan.id),numberdos_simple)
					x+= 1
			...
			return self.env['popup.it'].get_file('Flujo de Caja Proyectado de %s al %s.xlsx'%((str(self.date_from) if self.show_by == 'date' else self.period_from_id.name),(str(self.date_to) if self.show_by == 'date' else self.period_to_id.name)),base64.encodebytes(b''.join(f.readlines())))

	def get_saldo(self,type_date,use_counterpart_cash_flow,date_start,date_end,filtro,flujo_id=None,sign=None):
	...
	def _get_sql(self):
		...
		if not parameters.use_counterpart_cash_flow:
			sql = """SELECT 
					...
					aml.expected_pay_date as fecha,
					am.glosa,
					aml.account_id,
					...
					AND (aml.expected_pay_date BETWEEN '{date_from}' AND '{date_to}') )
