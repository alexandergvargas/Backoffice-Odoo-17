...
class HrVacation(models.Model):
	...
	line_ids = fields.One2many('hr.vacation.line', 'vacation_id', string='Calculo de Vacaciones')
	...
	def get_vacation(self):
		...
		for Employee in Employees:
			if Employee.contract_id.situation_id.code == '0':
				...
				leave_vacations = self.env['hr.work.suspension'].search([('periodo_id', '=', MonthLot.periodo_id.id),
																  ('employee_id', '=', Employee.id),
																  ('contract_id', '=', Employee.contract_id.id),
																  ('suspension_type_id', '=', suspension_type_id.id)], limit=1)

				number_of_days = self.env['hr.work.suspension'].search([('periodo_id', '=', MonthLot.periodo_id.id),
																  ('employee_id', '=', Employee.id),
																  ('contract_id', '=', Employee.contract_id.id),
																  ('suspension_type_id', '=', suspension_type_id.id)]).mapped('days')

				# print("leave_vacations",leave_vacations)
				if leave_vacations.periodo_id.id == MonthLot.periodo_id.id:
	...
	def get_vacation_sheet(self, workbook, lines):
		...
		widths = [13, 13, 13, 20, 10, 11, 11, 15, 15, 5, 5,
				  8, 11, 16, 13, 16, 14, 16, 11, 11, 10, 10,
				  8, 8, 8, 9, 9, 10, 12, 12]
...
class HrVacationLine(models.Model):
	...
	def compute_vacation_line(self):
		ReportBase = self.env['report.base']
		MonthLot = self.vacation_id.payslip_run_id
		...
		sql = """
				...
				FROM 
					...
					from (	SELECT	
							...
							group by he.id,he.name,hpr.periodo_id,hper.code,hp.date_from,hp.date_to,hsr.id,hsr.sequence
	...
	def view_detail_vac(self):
		...
		self.env.cr.execute(self._get_sql_salary_vac())
