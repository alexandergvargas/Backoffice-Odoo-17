...
class KardexEntryOutcomeWizard(models.TransientModel):
	...
	def get_excel(self):
		...
		return self.env['popup.it'].get_file(u'Detalle Salidas.xlsx',base64.encodebytes(b''.join(f.readlines())))
	...
	def make_invoice(self):
		...
		self.env.cr.execute("""DELETE FROM kardex_entry_outcome_book""")
		self.env.cr.execute(""" INSERT INTO kardex_entry_outcome_book (%s) (%s)"""%(','.join(i for i in self.values_move_insert()),self._get_sql_move(self.period.date_start,self.period.date_end,self.company_id.id)))
		
		if self.type_move == 'summary':
			self.env.cr.execute("""select tipo_op from kardex_entry_outcome_book group by tipo_op""")
	...
	def sql_values_to_generate_moves(self,type,is_debit=False,tipo_op=None):
		if type == 'summary':
			if is_debit:
				sql = """select tipo_name,cta_debe,analytic_account_id, sum(round(coalesce(amount,0),2)) as amount from kardex_entry_outcome_book
				where tipo_op = '%s'
				group by tipo_name,almacen,cta_debe,analytic_account_id"""%tipo_op
			else:
				sql = """select tipo_name,cta_haber,analytic_account_id,SUM(round(coalesce(amount,0),2)) as amount from kardex_entry_outcome_book
				where tipo_op = '%s'
				group by tipo_name,almacen,cta_haber,analytic_account_id"""%tipo_op
		else:
			sql = """select tipo_name,cta_debe,cta_haber,analytic_account_id, round(coalesce(amount,0),2) as amount from kardex_entry_outcome_book"""
