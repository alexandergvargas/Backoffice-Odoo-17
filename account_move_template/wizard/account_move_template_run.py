...
class AccountMoveTemplateRun(models.TransientModel):
    ...
    state = fields.Selection(
        [("select_template", "Seleccionar plantilla"), ("set_lines", "Establecer líneas")],
        readonly=True,
...
{'L1': {'partner_id': 1, 'amount': 100, 'name': 'some label'},
 'L2': {'partner_id': 2, 'amount': 200, 'name': 'some label 2'}, }
    ...
    nro_comp = fields.Char('Nro Comprobante')
    ...
    def _prepare_wizard_line(self, tmpl_line):
        vals = {
            ...
            "nro_comp": tmpl_line.nro_comp,
            "type_document_id": tmpl_line.type_document_id.id,
        }
        return vals
        ...
        self.write(
            {
                ...
                "state": "set_lines",
                "nro_comp": self.template_id.nro_comp,
                "type_document_id": self.template_id.type_document_id.id,
            }
        )
    ...
    def _prepare_move(self):
        move_vals = {
            ...
            "l10n_latam_document_type_id": self.type_document_id.id,
...
class AccountMoveTemplateLineRun(models.TransientModel):
    ...
    company_currency_id = fields.Many2one(
        related="wizard_id.company_id.currency_id", string=u"Moneda de la Compañia"
    )
    ...
    tax_line_id = fields.Many2one(
        "account.tax", string="Impuesto", ondelete="restrict", readonly=True
    )
    partner_id = fields.Many2one("res.partner", readonly=True, string="Cliente")
    payment_term_id = fields.Many2one(
        "account.payment.term", string="Terminos de pago", readonly=True
    )
    move_line_type = fields.Selection(
        [("cr", "Credito"), ("dr", "Debito")],
        required=True,
        readonly=True,
        string=u"Dirección",
    )
