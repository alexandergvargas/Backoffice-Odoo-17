...
class HrPlanillaTabularFortnightlyWizard(models.TransientModel):
	_name = 'hr.planilla.tabular.fortnightly.wizard'
	_description = 'Hr Planilla Tabular Fortnightly Wizard'

	name = fields.Char()
	company_id = fields.Many2one('res.company',string=u'Compañia',required=True, default=lambda self: self.env.company,readonly=True)
	fortnightly_id = fields.Many2one('hr.fortnightly', string='Periodo')
	employees_ids = fields.Many2many('hr.employee','hr_planilla_fortnightly_employee_rel','fortnightly_id','employee_id',string=u'Empleados')
	...
	def default_get(self, fields):
		res = super(HrPlanillaTabularFortnightlyWizard, self).default_get(fields)
		payslip_run_id = res.get('fortnightly_id')
		res.update({'fortnightly_id': payslip_run_id})
		return res
	...
	def _get_sql(self,option):
		...
		sql_payslips = "and hp.id in (%s)" % (','.join(str(i) for i in self.fortnightly_id.slip_ids.ids))
	...
	def _get_tab_payroll_sql(self,option):
		sql_employees = "and he.id in (%s) " % (','.join(str(i) for i in self.employees_ids.ids)) if option == 1 else ""
		struct_id = self.fortnightly_id.slip_ids[0].struct_id.id
		sql = """
			select
			he.id as employee_id,
			he.identification_id,
			...
			order by he.name, hsr.sequence
		""".format(
				ids = ','.join(list(map(str, self.fortnightly_id.slip_ids.ids))),
	...
	def get_excel(self,option):
		...
		workbook = Workbook(directory + 'Adelanto_Quincenal_Tabular.xlsx')
		...
		worksheet = workbook.add_worksheet("Adelanto Quincenal Tabular")
		...
		worksheet.merge_range(4, 1, 4, 8, "*** ADELANTO DE SUELDOS Y SALARIOS %s ***"  % self.fortnightly_id.name or '', formats['especial5'])
		...
		workbook.close()
		f = open(directory + 'Adelanto_Quincenal_Tabular.xlsx', 'rb')
		return self.env['popup.it'].get_file('Adelanto %s.xlsx' % self.fortnightly_id.name, base64.encodebytes(b''.join(f.readlines())))
