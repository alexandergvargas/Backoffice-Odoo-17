...
class HrLoanType(models.Model):
	...
	def get_loan_wizard(self):
		wizard = self.env['hr.advance.loan.wizard'].create({'name':'Duplicar Tipos de Prestamo'})
		return {
			'type':'ir.actions.act_window',
			'res_id':wizard.id,
			'view_mode':'form',
			'res_model':'hr.advance.loan.wizard',
			'views':[[self.env.ref('hr_advances_and_loans.hr_duplicate_loan_wizard_form').id,'form']],
...
class HrLoan(models.Model):
	...
	date = fields.Date(string='Fecha de Prestamo',tracking=True)
	amount = fields.Float(string='Monto de Prestamo',tracking=True)
	loan_type_id = fields.Many2one('hr.loan.type', string='Tipo de Prestamo')
	fees_number = fields.Integer(string='Numero de Cuotas',tracking=True)
