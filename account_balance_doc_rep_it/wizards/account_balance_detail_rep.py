	...
	def get_report(self):
		...
		if self.type_account:
			if self.type_account == 'others':
				sql_type_acc = """and a2.account_type not in ('liability_payable','asset_receivable')"""
			else:
				sql_type_acc = """and a2.account_type = '%s'""" % (self.type_account)


		sql = """
			CREATE OR REPLACE view account_balance_detail_book as (SELECT row_number() OVER () AS id,a1.* FROM get_saldo_detalle('%s'::date,'%s'::date,%d::integer) a1 
