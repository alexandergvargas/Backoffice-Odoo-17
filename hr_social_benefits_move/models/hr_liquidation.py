...
union all
select distinct
    CASE WHEN hecl.type= 'in' THEN prm.account_debit ELSE prm.account_credit END AS account_id,
    hsr.name->>'en_US' as description,
    CASE WHEN hecl.type= 'in' THEN hadl.analytic_id ELSE null::integer END AS analytic_account_id,
...
from hr_extra_concept_line hecl
         ...
         inner join payslip_run_move prm ON prm.salary_rule_id = hsr.id
...
where hsr.company_id = {company_id}
  ...
  and hecl.amount <> 0
  and (prm.account_debit is not null or prm.account_credit is not null)
						""".format(
            cts_haber= MainParameter.cts_haber.id,
            grati_haber=MainParameter.grati_haber.id,
            vaca_haber=MainParameter.vaca_haber.id,
    ...
    def get_liquidation_move_wizard(self):
        ...
        return {
            ...
            'views': [(self.env.ref('hr_social_benefits_move.hr_liquidation_move_wizard_form').id, 'form')],
