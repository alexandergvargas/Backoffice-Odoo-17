...
select
    ...
    hpl.employee_id,
    he.work_location_id,
    hsr.account_debit as account_id,
...
left join account_account aa on hsr.account_debit = aa.id
...
from (
         ...
         from hr_gratification_line hgl
                  ...
                  left join hr_employee he on he.id = hc.employee_id
                  left join account_account aa on {grati_haber} = aa.id
                  left join res_partner rp on he.user_partner_id = rp.id
...
from (
         ...
         FROM hr_gratification_line hgl
                  ...
                  inner join hr_salary_rule hsr on hsr.id = (select id from hr_salary_rule where code='GRA' and company_id = {company})
                  left join account_analytic_account aaa on aaa.id = hadl.analytic_id
                  left join account_account aa on hsr.account_debit = aa.id
                  left join res_partner rp on he.user_partner_id = rp.id
         where hg.company_id = {company}
...
from (
         ...
         from hr_cts_line hcl
	        ...
	        left join hr_employee he on he.id = hc.employee_id
	        left join account_account aa on {cts_haber} = aa.id
            left join res_partner rp on he.user_partner_id = rp.id
...
from (
         ...
         FROM hr_cts_line hcl
                  ...
                  inner join hr_analytic_distribution had on had.id = hc.distribution_id
	              inner join hr_analytic_distribution_line hadl on hadl.distribution_id = had.id
	              inner join hr_salary_rule hsr on hsr.id = (select id from hr_salary_rule where code='CTS' and company_id = {company})
	              left join account_analytic_account aaa on aaa.id = hadl.analytic_id
                  left join account_account aa on hsr.account_debit = aa.id
                  left join res_partner rp on he.user_partner_id = rp.id
         where hcts.company_id = {company}
