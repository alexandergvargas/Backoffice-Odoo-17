...
class AccountMove(models.Model):
	...
	def button_cancel(self):
		if self.move_free_transfer_id.id:
			...
			self.move_free_transfer_id.vou_number = "/"
	...
	def make_move_free_transfer(self,lines):
		...
		if len(self.line_ids.filtered(lambda l: l.account_id.account_type in ('asset_receivable','liability_payable'))) > 1:
			raise UserError(u'En el caso de tener una factura con diferentes Plazos de pago, las trasnferencias gratuitas se realizan en el menú: Tesoreria / Tesoreria / Pagos Múltiples')
		filtered_line = self.line_ids.filtered(lambda l: l.account_id.account_type in ('asset_receivable','liability_payable'))
		values = (0,0,{
