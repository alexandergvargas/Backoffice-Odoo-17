...
class ImportJournalEntryIt(models.Model):
	...
	def make_move_line(self,values,move_id):
		...
		json_analytic_distribution = None
		
		if values.get("analytic_distribution"):
			distributions=str(values.get("analytic_distribution")).split(';')
			distribution_dict = {}
			for distribution in distributions:
				split_distribution=distribution.split('(')
				
				try:
					analytic_tag=str(split_distribution[0])
				except:
					raise UserError(f"Cuenta Analitica Incorrecta en {split_distribution[0]}")
				try:
					percentage=float(split_distribution[1][:-2])
				except:
					raise UserError(f"Porcentaje Incorrecto en {split_distribution[1:-2]}")
				distribution_dict[analytic_tag]=percentage
			json_analytic_distribution = self.find_analytic_distribution(distribution_dict)
		...
		if currency.name != 'PEN':
			vals = {
				...
				'analytic_distribution': json_analytic_distribution if json_analytic_distribution else None,
	...
	def find_analytic_distribution(self, distribution_dict):
		json_result={}
		analytic_obj = self.env['account.analytic.account']
		for analytic,percentage in distribution_dict.items():
			analytic_search = analytic_obj.search([
       			('code', '=', str(analytic)),
          		('company_id','=',self.company_id.id)
            ],limit=1)
			if not analytic_search:
				raise UserError(_(f'No existe una Cuenta Analitica con la Referencia {analytic} en esta Compañia'))
			json_result[str(analytic_search)]=percentage
		return json_result
