...
class ImportDetracWizard(models.TransientModel):
	...
	def add_doc_relac_move(self, values):
		...
		if str(values.get('serie')) == '':
			raise UserError(_('El campo "Serie de Comprobante" no puede estar vacio.'))
		if str(values.get('ref')) == '':
			raise UserError(_('El campo "Numero de Comprobante" no puede estar vacio.'))
		if str(values.get('ruc')) == '':
			raise UserError(_('El campo "RUC Proveedor" no puede estar vacio.'))
		if str(values.get('type_op_det')) == '':
			raise UserError(_('El campo "Tipo de Operación" no puede estar vacio.'))
		if str(values.get('detraction_percent_id')) == '':
			raise UserError(_('El campo "Bien o Servicio" no puede estar vacio.'))
		...
		detraction_percent_id = self.env['detractions.catalog.percent'].search([
      		('code','=',str(values.get('detraction_percent_id')))
        ],limit=1)
		if not detraction_percent_id:
			raise UserError(f"No existe un Bien o Servicio con el codigo '{values.get('detraction_percent_id')}'")
		#
		if not move_search:
			raise UserError(f"No se encontró el asiento: '{ruc}' - '{doc.code}' - '{serie+'-'+ref}' - '{self.journal_id.name}'")
		move_search.write({
			'l10n_pe_detraction_date': values.get("date") if values.get("date") else None,
			'l10n_pe_detraction_number': nro_comprobante,
