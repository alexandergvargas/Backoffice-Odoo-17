...
class HrMainParameter(models.Model):
	...
	# net_sr_id = fields.Many2one('hr.salary.rule', string='R. S. Neto Rem.', required=True)
	...
	insurable_remuneration = fields.Many2one('hr.salary.rule', string='R. S. Ingresos Afectos AFP', required=True)
	wd_dlab = fields.Many2many('hr.work.entry.type', 'wd_dlab_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Dias Laborados')
	wd_dnlab = fields.Many2many('hr.work.entry.type', 'wd_dnlab_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Dias no Laborados')
	wd_dsub = fields.Many2many('hr.work.entry.type', 'wd_dsub_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Dias Subsidiados')
	wd_ext = fields.Many2many('hr.work.entry.type', 'wd_dext_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Sobretiempo')
	wd_dvac = fields.Many2many('hr.work.entry.type', 'wd_dvac_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Ausencias')
	income_categories = fields.Many2many('hr.salary.rule.category', 'src_income_main_parameter_rel', 'main_parameter_id', 'src_id', string='Categoria Ingresos')
	discounts_categories = fields.Many2many('hr.salary.rule.category', 'src_discounts_main_parameter_rel', 'main_parameter_id', 'src_id', string='Categoria Descuentos')
	contributions_categories = fields.Many2many('hr.salary.rule.category', 'src_contributions_main_parameter_rel', 'main_parameter_id', 'src_id', string='Categoria Aportes Trabajador')
	contributions_emp_categories = fields.Many2many('hr.salary.rule.category', 'src_contributions_emp_main_parameter_rel', 'main_parameter_id', 'src_id', string='Categoria Aportes Empleador')
	signature = fields.Binary(string='Firma Empleador')
	# payslip_working_wd = fields.Many2one('hr.work.entry.type', string='W.D. Dias Laborados')
