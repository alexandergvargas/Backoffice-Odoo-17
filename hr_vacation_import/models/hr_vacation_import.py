	...
	def vacation_import(self):
		...
		for row_no in range(sheet.nrows):
			...
			if row_no <= 0:
				...
				empl_exist = self.env['hr.employee'].search(
					[('identification_id', '=', cad), ('company_id', '=', self.env.company.id)])

				if not empl_exist:
					caderror = caderror + 'No existe el empleado identificado con: ' + cad + '\n'
					continue

				saldo_exist = self.env['hr.vacation.rest'].search(
					[('identification_id', '=', cad), ('internal_motive', '=', 'rest'),
					 ('company_id', '=', self.env.company.id)])
				# print("saldo_exist",saldo_exist)
				if saldo_exist:
					saldo_exist.unlink()
