...
class ImportMoveAperturaIt(models.Model):
	...
	def importar(self):
		...
		for row_no in range(1,sheet.nrows):
			...
			if len(line) > 13:
				raise UserError('Tu archivo tiene columnas más columnas de lo esperado.')
			if len(line) < 13:
		...
		self.env.cr.execute(f"""
			...
			from (
				...
				from 
					import_move_apertura_it_line iaa
					...
					left join account_account aa on aa.code = iaa.n_cuenta and aa.company_id = {self.company_id.id}
				where iaa.wizard_id = {self.id}
    		) AS T 
      		where 
        		T.id = import_move_apertura_it_line.id
		 """)
		...
		self.env.cr.execute(f"""
            ...
            where 
            	wizard_id = {self.id}
			group by 
   				n_ruc,n_tipo_doc,n_numero,n_cuenta
			having 
   				count(n_ruc)>1""")

		for i in self.env.cr.fetchall():
			problemas+= f"Existen {str(i[0])} lineas duplicadas con los siguientes datos: \n"
			problemas+= f"RUC: {str(i[1])} \n"
			problemas+= f"Tipo Documento: {str(i[2])} \n"
			problemas+= f"Nro Comprobante: {str(i[3])} \n"
			problemas+= f"Cuenta: {str(i[4])} \n \n"
		...
		for acc in data:
			...
			if self.tipo == 'out':
				...
				amount_mn = acc[7] if type_invoice == 'out_invoice' else abs(acc[7])*-1
				amount_me = acc[8]*-1 if type_invoice == 'out_invoice' else abs(acc[8])
				...
				vals = (0,0,{
					...
					'amount_currency': amount_me if currency.name != 'PEN' else amount_mn*-1,
					'debit': 0 if amount_mn > 0 else abs(amount_mn),
					'credit': amount_mn if amount_mn > 0 else 0,
					'price_subtotal':acc[8] if currency.name != 'PEN' else abs(amount_mn),
					'price_total':acc[8] if currency.name != 'PEN' else abs(amount_mn),
					'price_unit':acc[8] if currency.name != 'PEN' else abs(amount_mn),
					'date_maturity':acc[2],
				...
				vals = (0,0,{
					...
					'name': 'SALDOS DE APERTURA',
					'currency_id': currency.id if currency.name != 'PEN' else self.company_id.currency_id.id,
					'amount_currency': amount_me*-1 if currency.name != 'PEN' else amount_mn,
				...
				amount_mn = acc[7] if type_invoice == 'in_invoice' else abs(acc[7])*-1
				amount_me = acc[8] if type_invoice == 'in_invoice' else abs(acc[8])*-1
				vals = (0,0,{
					'account_id': self.account_descargo_me.id if currency.name != 'PEN' else self.account_descargo_mn.id,
					'partner_id': self.partner_descargo.id,
					'type_document_id':document_code.id,
					...
					'name': 'SALDOS DE APERTURA',
					'currency_id': currency.id if currency.name != 'PEN' else self.company_id.currency_id.id,
					'amount_currency': amount_me if currency.name != 'PEN' else amount_mn,
					'debit': amount_mn if amount_mn > 0 else 0,
					'credit': 0 if amount_mn > 0 else abs(amount_mn),
					'price_subtotal':acc[8] if currency.name != 'PEN' else abs(amount_mn),
					'price_total':acc[8] if currency.name != 'PEN' else abs(amount_mn),
				...
				vals = (0,0,{
					...
					'name': 'SALDOS DE APERTURA',
					'currency_id': currency.id if currency.name != 'PEN' else self.company_id.currency_id.id,
					'amount_currency': amount_me*-1 if currency.name != 'PEN' else amount_mn*-1,
					'debit': 0 if amount_mn > 0 else abs(amount_mn),
					'credit': amount_mn if amount_mn > 0 else 0,
					'price_subtotal':acc[8]*-1 if currency.name != 'PEN' else amount_mn*-1,
					'price_total':acc[8]*-1 if currency.name != 'PEN' else amount_mn*-1,
			...
			'amount_untaxed': acc[8] if currency.name != 'PEN' else abs(amount_mn),
			'amount_total': acc[8] if currency.name != 'PEN' else abs(amount_mn),
			'amount_residual': acc[8] if currency.name != 'PEN' else abs(amount_mn),
			...
			move_id._get_ref()
			for line in move_id.line_ids.filtered(lambda l: l.account_id.id == (self.account_descargo_me.id if currency.name != 'PEN' else self.account_descargo_mn.id)):
				line.partner_id = self.partner_descargo.id
		self.state = 'import'
