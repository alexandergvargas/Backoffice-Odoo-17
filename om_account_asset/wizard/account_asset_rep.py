...
class AccountAssetRep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
			CREATE OR REPLACE view account_asset_book as ("""+self._get_sql(self.period_from.date_start,self.period_to.date_end,self.company_id.id,self.currency_id.name)+""")""")
	...
	def _get_sql(self,x_date_ini,x_date_end,x_company_id,x_currency):
		if x_currency == 'PEN':
			sql = """select id, code, name, mes, period, cat_name, cta_analitica, cta_activo, cta_gasto, cta_depreciacion, valor_dep
