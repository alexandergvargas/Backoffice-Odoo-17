...
class AccountAsset71Rep(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in self.env['account.asset.71.book'].search([]):
			...
			worksheet.write(x,14,line.campo15 if line.campo15 else '',formats['reverse_dateformat'])
			worksheet.write(x,15,line.campo16 if line.campo16 else '',formats['reverse_dateformat'])
	...
	def getPdf(self):
		...
		return self.env['popup.it'].get_file('Libro de Activos Formato 7_1',base64.encodebytes(b''.join(f.readlines())))

	def get_ple(self,move_id):
		...
		sql_ple = self._get_sql_ple(self.fiscal_year_id.date_from,self.period.date_start,self.period.date_end,self.company_id.id,self.period.code,move_id)
		ReportBase = self.env['report.base']
	...
	def get_excel_ple(self,move_id):
		ReportBase = self.env['report.base']
		sql_ple = self._get_sql_ple(self.fiscal_year_id.date_from,self.period.date_start,self.period.date_end,self.company_id.id,self.period.code,move_id)
		workbook = ReportBase.get_excel_sql_export(sql_ple,['CAMPO 1','CAMPO 2','CAMPO 3','CAMPO 4','CAMPO 5','CAMPO 6','CAMPO 7','CAMPO 8','CAMPO 9','CAMPO 10','CAMPO 11','CAMPO 12',
					...
					'CAMPO 35','CAMPO 36','CAMPO 37'])
		return self.env['popup.it'].get_file('PLE 7.1.xlsx',workbook)
	...
	def _get_sql_ple(self,date_fiscal_year_start,date_period_start,date_period_end,company_id,period_code,move_id):
		sql = """
				select 
				...
				0.00::numeric as campo17,
				T.campo10 as campo18,
				0.00::numeric as campo19,
				0.00::numeric as campo20,
				0.00::numeric as campo21,
				...
				T.campo21 as campo30,
				T.campo22 as campo31,
				0.00::numeric as campo32,
				0.00::numeric as campo33,
				0.00::numeric as campo34,
				0.00::numeric as campo35,
				0.00::numeric as campo36,
				T.campo37_ple as campo37,
				NULL as campo38
				from
				(select asset.code,
				aa.code as cuenta,
				left(REGEXP_REPLACE(asset.name, '[^a-zA-Z0-9 ]', '', 'g'),40) as asset_name,
				case
					when asset.date < '{date_fiscal_year_start}' then asset.value
					else 0.00::numeric
				end
				as campo7,
				case
					when asset.date >= '{date_fiscal_year_start}' then round(asset.value,2)
					else 0.00::numeric
				end
				as adq_adi,
				0.00::numeric as campo9,
				
				case when asset.f_baja <= '{date_period_end}' then (asset.value)*-1 else 0::numeric end as campo10,
				0.00::numeric as campo11,
				0.00::numeric as campo13,
				...
				case when '{period}' = '00' then 0::numeric else round(coalesce(t2.campo21,0),2) end as campo21,
				case when asset.f_baja <= '{date_period_end}' then (round(coalesce(t1.campo20,0),2) + (case when '{period}' = '00' then 0::numeric else round(coalesce(t2.campo21,0),2) end))*-1 end as campo22,
				asset.id as asset_id,
				asset.id,
				(CASE WHEN asset.date < '{date_fiscal_year_start}' THEN (CASE
					WHEN ami.is_opening_close = true AND to_char(ami.date::timestamp with time zone, 'mmdd'::text) = '0101'::text THEN 'A'
					WHEN ami.is_opening_close = true AND to_char(ami.date::timestamp with time zone, 'mmdd'::text) = '1231'::text THEN 'C'
					ELSE 'M' END) ELSE (CASE
					WHEN am.is_opening_close = true AND to_char(am.date::timestamp with time zone, 'mmdd'::text) = '0101'::text THEN 'A'
					WHEN am.is_opening_close = true AND to_char(am.date::timestamp with time zone, 'mmdd'::text) = '1231'::text THEN 'C'
					ELSE 'M' END) END ) || (case when asset.date < '{date_fiscal_year_start}' then ami.name else am.name end)
				AS campo3,
				asset.table_13_ple,
				asset.table_18_ple,
				asset.table_19_ple,
				case when asset.brand <> '' THEN left(coalesce(asset.brand,'-'),20) ELSE '-' END as brand,
				case when asset.model <> '' THEN left(coalesce(asset.model,'-'),20) ELSE '-' END as model,
				case when asset.plaque <> '' THEN left(coalesce(asset.plaque,'-'),30) ELSE '-' END as plaque,
				asset.table_20_ple,
				coalesce(asset.depreciation_authorization,'NS') as depreciation_authorization,
				round(asset.depreciation_rate::numeric,2) as depreciation_rate,
				asset.campo37_ple
				from account_asset_asset asset
				left join account_asset_category cat on cat.id = asset.category_id
				left join account_account aa on aa.id = cat.account_asset_id
				left join (select asset_id, sum(amount) as campo20 from account_asset_depreciation_line 
				where depreciation_date < '{date_fiscal_year_start}'
				group by asset_id)t1 on t1.asset_id = asset.id
				left join (select asset_id, sum(amount) as campo21 from account_asset_depreciation_line 
				where (depreciation_date between '{date_fiscal_year_start}' and '{date_period_end}')
				group by asset_id)t2 on t2.asset_id = asset.id
				left join account_move am on am.id = asset.invoice_id
				left join account_move ami on ami.id = {move_id}
				where asset.company_id = {company_id} and (asset.only_format_74 = False or asset.only_format_74 is null) and asset.state <> 'draft'
				and asset.date <= '{date_period_end}' and (asset.f_baja is null or asset.f_baja >= '{date_fiscal_year_start}'))T
		""".format(period_code = period_code[:4]+'0000',
		date_fiscal_year_start = date_fiscal_year_start.strftime('%Y/%m/%d'),
		period = period_code[4:],
		date_period_end = date_period_end.strftime('%Y/%m/%d'),
		date_period_start = date_period_start.strftime('%Y/%m/%d'),
		move_id = move_id,
