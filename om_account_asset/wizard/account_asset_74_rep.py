...
class AccountAsset74Rep(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in self.env['account.asset.74.book'].search([]):
			...
			worksheet.write(x,1,line.campo2 if line.campo2 else '',formats['reverse_dateformat'])
			worksheet.write(x,2,line.campo3 if line.campo3 else '',formats['especial1'])
			worksheet.write(x,3,line.campo4 if line.campo4 else '',formats['reverse_dateformat'])
	...
	def _get_sql_ple(self,period,date_period_start,date_period_end,company_id):
		sql = """select 
				...
				asset.cuo as campo2,
				CASE
					WHEN am.is_opening_close = true AND to_char(am.date::timestamp with time zone, 'mmdd'::text) = '0101'::text THEN 'A' || am.vou_number
					WHEN am.is_opening_close = true AND to_char(am.date::timestamp with time zone, 'mmdd'::text) = '1231'::text THEN 'C' || am.vou_number
					ELSE 'M' || am.vou_number
				END AS campo3,
