...
class WorksheetF1Wizard(models.TransientModel):
	...
	def _get_f1_register_sql(self):
		sql = """
			...
			a2.code as cuenta,
			(a2.name->>'es_PE'::character varying)::character varying as nomenclatura,
			a1.debe,
			a1.haber,
			...
			ati.name as rubro,
			CASE 
				WHEN ati.group_balance = 'B1' THEN 'Activo Corriente'::character varying 
				WHEN ati.group_balance = 'B2' THEN 'Activo no Corriente'::character varying 
				WHEN ati.group_balance = 'B3' THEN 'Pasivo Corriente'::character varying 
				WHEN ati.group_balance = 'B4' THEN 'Pasivo no Corriente'::character varying 
				WHEN ati.group_balance = 'B5' THEN 'Patrimonio'::character varying 
			END AS group_balance,
			CASE 
				WHEN ati.group_nature = 'N1' THEN 'Grupo 1'::character varying 
				WHEN ati.group_nature = 'N2' THEN 'Grupo 2'::character varying 
				WHEN ati.group_nature = 'N3' THEN 'Grupo 3'::character varying 
				WHEN ati.group_nature = 'N4' THEN 'Grupo 4'::character varying 
				WHEN ati.group_nature = 'N5' THEN 'Grupo 5'::character varying 
				WHEN ati.group_nature = 'N6' THEN 'Grupo 6'::character varying 
				WHEN ati.group_nature = 'N7' THEN 'Grupo 7'::character varying 
				WHEN ati.group_nature = 'N8' THEN 'Grupo 8'::character varying 
			END AS group_nature,
			CASE 
				WHEN ati.group_function = 'F1' THEN 'Grupo 1'::character varying 
				WHEN ati.group_function = 'F2' THEN 'Grupo 2'::character varying 
				WHEN ati.group_function = 'F3' THEN 'Grupo 3'::character varying 
				WHEN ati.group_function = 'F4' THEN 'Grupo 4'::character varying 
				WHEN ati.group_function = 'F5' THEN 'Grupo 5'::character varying 
				WHEN ati.group_function = 'F6' THEN 'Grupo 6'::character varying 
			END AS group_function
			...
			null::character varying as group_balance,
			null::character varying as group_nature,
			null::character varying as group_function
			from get_sumas_mayor_f1('{date_from}','{date_to}',{company_id},{show_closed}) a1
			left join account_account a2 on a2.id=a1.account_id)
			UNION ALL
			(
	...
	def get_excel_f1_register(self):
		...
		if self.show_account_entries:
			...
			HEADERS.append('GRUPO BALANCE')
			HEADERS.append('GRUPO NATURALEZA')
			HEADERS.append('GRUPO FUNCION')
		...
		for c,line in enumerate(res):
			...
			if self.show_account_entries:
				...
				worksheet.write(x,14,line['group_balance'] if line['group_balance'] else '',formats['especial1'])
				worksheet.write(x,15,line['group_nature'] if line['group_nature'] else '',formats['especial1'])
				worksheet.write(x,16,line['group_function'] if line['group_function'] else '',formats['especial1'])
			x += 1
