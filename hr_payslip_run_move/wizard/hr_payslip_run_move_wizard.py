	...
	def generate_move(self):
		...
		if self.debit > self.credit:
			extra_line = {
				...
				'nro_comp' : ('PLA'+PR.periodo_id.code[4:6]+PR.periodo_id.code[0:4]),
		...
		move = self.env['account.move'].create({
			...
			'date': PR.date_end,
			'glosa': 'PLANILLA DE REMUNERACIONES '+(PR.name),
			'ref': ('PLA'+PR.periodo_id.code[4:6]+PR.periodo_id.code[0:4]),
			'line_ids': extra_line + [
				(0, 0, {
					...
					'credit': line.credit,
					'type_document_id' : MainParameter.type_doc_pla.id,
					'nro_comp' : ('1QUIN'+PR.periodo_id.code[4:6]+PR.periodo_id.code[0:4]) if line.salary_rule_id.code == 'ADE_QUIN' else ('PLA'+PR.periodo_id.code[4:6]+PR.periodo_id.code[0:4]),
					'name': line['description'] if line['description'] else None,
					'analytic_distribution': {line['analytic_account_id'].id: 100} if line['analytic_account_id'].id else None,
